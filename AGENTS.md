...
| `tests/` | 单测：`parser.test.ts` 契约快照与协议头断言、`crafting.test.ts` 章节制作流水线与 store 公共 API、`server.test.ts` 世界线/协议解析/快照子系统/音频扫描、`treeLayout.test.ts` 布局与视图纯函数、`ui.test.tsx` 组件（jsdom）、`contract.test.ts` 契约 lint（v1.6 防漂移门禁：协议头唯一真源/RULES 逐字副本/指令字符串双处/用例数与文档比对/设置键与音频扩展名，自身不计入 302 口径）；`setup-react-act.mjs` 为 react 19.3 缺失 `React.act` 打测试垫片；`e2e/smoke.spec.ts` 真引擎冒烟；`helpers/stack.mjs` 进程编排；`tests/e2e-ui/`（9 个 spec：`opening`/`settings`/`keyboard`/`gallery`/`story-tree`/`worlds`/`restore`/`reduced-motion`/`focus` + `flow.ts` 屏幕流 helper，`playwright.ui.config.ts` 默认 chromium）假引擎确定性 UI e2e，进程由 `helpers/fake-stack.mjs` 编排（复用 `tests/integration/harness.mjs` 起假引擎+真 acp-server，再起 vite dev 代理过去；harness 的 seed 项 assets/audioFiles/trees/worlds/snapshots 在此消费） |
...
- `npm test`（`vitest run --exclude "tests/e2e/**" --exclude "tests/e2e-ui/**"`）：parser 65 + server 76 + crafting 52 + treeLayout 7 + ui 83 + integration 19（pipeline 8 + audio-history 8 + http-guard 3），**共 302 例**，秒级——默认含集成层，改契约字符串必须同步改快照；另跑契约 lint `tests/contract.test.ts`（v1.6 防漂移门禁，**不计入这 302**——它断言的就是上面这些数字与协议真源）。

...
| `src/lib/acp.ts` | HTTP/SSE 客户端与 `AcpEvent` / `Preset` / `WorldEntry` / `WorldSnapshot` / `WorldBundle` / `AudioItem` / `StateView` 类型，`fetchWorlds`/`postWorld`/`fetchTree`（世界线与剧情图屏）、`fetchHistory`/`worldExportUrl`/`postWorldImport`/`postWorldUpdate`（快照、导出导入、改名）、`fetchAudio`/`audioFileUrl`（音频索引与直服 URL）、`fetchState`（角色面板的 state.md 解析视图） |
...
- `npm test`（`vitest run --exclude "tests/e2e/**" --exclude "tests/e2e-ui/**"`）：parser 65 + server 89 + crafting 52 + treeLayout 7 + ui 98 + integration 19（pipeline 8 + audio-history 8 + http-guard 3），**共 330 例**，秒级——默认含集成层，改契约字符串必须同步改快照；另跑契约 lint `tests/contract.test.ts`（v1.6 防漂移门禁，**不计入这 330**——它断言的就是上面这些数字与协议真源）。

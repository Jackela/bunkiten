...
- `npm test`（`vitest run --exclude "tests/e2e/**" --exclude "tests/e2e-ui/**"`）：parser 65 + server 89 + crafting 52 + treeLayout 7 + genealogy 8 + ui 102 + integration 19（pipeline 8 + audio-history 8 + http-guard 3），**共 342 例**，秒级——默认含集成层，改契约字符串必须同步改快照；另跑契约 lint `tests/contract.test.ts`（v1.6 防漂移门禁，**不计入这 342**——它断言的就是上面这些数字与协议真源）。

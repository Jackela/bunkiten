// 「引擎与密钥」设置节里**一组**凭据的表单（对话或出图）（v1.13 从 EngineKeysSection 抽出）。
//
...
import { Eye, EyeOff, Plug, Trash2 } from "lucide-react";
import {
  postCredentials,
  testCredentials,
...
/** 哪一组（对话 / 出图） */
export type GroupKey = "llm" | "image";
 ...
 */
export function EngineGroupForm({
  group,
  view,
  catalog,
  locked,
  ...
  // 依赖逐字段列全（含 sizeBackground）：effect 里只碰这几个字段，不引用整个 view 对象——
  // 从前 deps 漏了 sizeBackground、又靠 view 对象取值，exhaustive-deps 因此报警（v1.13 收口）。
  const { mode, provider, baseUrl, model, size, sizeBackground } = view;
  useEffect(() => {
  ...
  }, [mode, provider, baseUrl, model, size, sizeBackground]);

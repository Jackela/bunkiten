...
bunkiten（分岐点）是一个**本机运行**的开源桌面应用（MIT）：Electron 壳 + React 前端，叙事引擎跑在你自己的机器上（ACP 协议），两个后端任选——本机的 `grok` CLI，或随包分发的 `codex-acp`（跑在游戏自己的 `CODEX_HOME`，复用你终端里的 ChatGPT 登录态）。它没有我们运营的服务端，不收集遥测，不上传你的剧本、存档或对话。

因此，安全责任边界是清楚的：

- **你的本机数据由你自理**：`state/`（进度、摘要、剧情树）与 `presets/`（剧本与美术资产）都写在你的磁盘上，请自行备份。
- **API key / 登录凭据由你自理，但要清楚它们落在哪**：v1.10 起游戏自己会写 `~/.bunkiten/credentials.json`（自备密钥与所选引擎，目录 0700 / 文件 0600，**明文**——本机磁盘加密是第一道防线），v1.11 起还在 `~/.bunkiten/codex/`（`CODEX_HOME`）放一份你 `~/.codex/auth.json` 的 0600 拷贝。两者都**不进仓库、不进世界线、不进任何导出包**，HTTP 出口一律脱敏；终端登录态仍归各个 CLI 自己。报告问题时，请先删掉日志、配置、截图里的 key、token 和带用户名的绝对路径。

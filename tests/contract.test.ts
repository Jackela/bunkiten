...
// 注意：本文件自身也被 `npm test` 收录，但**不计入**文档声明的 330 例口径（tests/e2e/** 同样不在口径内），见第 ④ 组。
...
/** 文档口径的分组（README / AGENTS / ARCHITECTURE 三处声明），合计 330 例；不含 e2e 与本文件 */
const CASE_GROUPS = [
  ...
  { name: "ui", files: ["tests/ui.test.tsx"], declared: 98 },
  {
    name: "integration",
...
/** 契约 lint 自己（也被 npm test 收录，但按文档口径**不计入** 316） */
const CONTRACT_FILE = "tests/contract.test.ts";

/** 文档声明的合计口径 */
const CASE_TOTAL = 330;
...
const DOC_GROUP_RES: { re: RegExp; group: (m: RegExpExecArray) => string }[] = [
  { re: /\b(parser|crafting|server|treeLayout|ui|integration)[ \t]+(\d+)\b/g, group: (m) => m[1] },
  { re: /\b(parser|crafting|server|treeLayout|ui|integration)(?:\.test\.(?:ts|tsx)|\/)[^\n]{0,80}?(\d+)\s*例/g, group: (m) => m[1] },
...
describe("④ 用例数：文档声明的分组数字 ↔ 各文件实际用例数", () => {
  ...
  it("三份文档里出现的每个分组数字都等于实际用例数（宽松解析：允许「例」与加号列）", () => {
    ...
    for (const doc of DOCS) {
      ...
      expect(
        missing,
        `${doc} 没有声明这些分组的用例数（分组口径必须六项齐全，好让 lint 逐项比对）：${missing.join("、")}——现在只声明了 ${[...seen].sort().join("、")}`,
      ).toEqual([]);
    }
  });

  it("文档里的子分组数字与合计（「共/全量 N 例」）都等于 330 口径", () => {
    let totals = 0;
    for (const doc of DOCS) {
      const text = read(doc);

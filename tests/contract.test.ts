...
/** 文档口径的分组（README / AGENTS / ARCHITECTURE 三处声明），合计 342 例；不含 e2e 与本文件 */
const CASE_GROUPS = [
  { name: "parser", files: ["tests/parser.test.ts"], declared: 65 },
  { name: "server", files: ["tests/server.test.ts"], declared: 89 },
  ...
  { name: "ui", files: ["tests/ui.test.tsx"], declared: 102 },
  {
    name: "integration",
...
/** 契约 lint 自己（也被 npm test 收录，但按文档口径**不计入** 342） */
const CONTRACT_FILE = "tests/contract.test.ts";

/** 文档声明的合计口径 */
const CASE_TOTAL = 342;
...
const DOC_GROUP_RES: { re: RegExp; group: (m: RegExpExecArray) => string }[] = [
  { re: /\b(parser|crafting|server|treeLayout|genealogy|ui|integration)[ \t]+(\d+)\b/g, group: (m) => m[1] },
  { re: /\b(parser|crafting|server|treeLayout|genealogy|ui|integration)(?:\.test\.(?:ts|tsx)|\/)[^\n]{0,80}?(\d+)\s*例/g, group: (m) => m[1] },
...
describe("④ 用例数：文档声明的分组数字 ↔ 各文件实际用例数", () => {
  ...
  it("七个分组合计等于文档口径 342，且本文件不计入其中", () => {
    const sum = CASE_GROUPS.reduce((n, g) => n + g.declared, 0);
    expect(sum, `文档的分组口径自相矛盾：七个分组相加 ${sum} 例，文档合计写的是 ${CASE_TOTAL} 例`).toBe(CASE_TOTAL);
  ...
  it("三份文档里出现的每个分组数字都等于实际用例数（宽松解析：允许「例」与加号列）", () => {
    ...
    for (const doc of DOCS) {
      ...
      expect(
        missing,
        `${doc} 没有声明这些分组的用例数（分组口径必须七项齐全，好让 lint 逐项比对）：${missing.join("、")}——现在只声明了 ${[...seen].sort().join("、")}`,
      ).toEqual([]);
    }
  });

  it("文档里的子分组数字与合计（「共/全量 N 例」）都等于 342 口径", () => {
    let totals = 0;
    for (const doc of DOCS) {
      const text = read(doc);

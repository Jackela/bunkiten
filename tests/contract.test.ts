...
import {
  DIRECTIVE_PREFIX_RE as SHARED_DIRECTIVE_RE,
  PROTOCOL_HEADS as SHARED_HEADS,
  CHAPTER_MARK_RE as SHARED_CHAPTER_RE,
...
const HEAD_PROBES: { head: string; from: string; name: string; sample: string; ok: (out: unknown) => boolean }[] = [
  {
    ...
    name: "parseArtLine",
    sample: "【图】立绘|薇拉|images/1.jpg",
    ok: (o) => (o as { name?: string } | null)?.name === "薇拉",
  },
  {
    head: "清单",
    ...
    name: "parseManifest",
    sample: "【清单】立绘|薇拉",
    ok: (o) => Array.isArray(o) && o.length === 1,
  },
  { head: "章", from: "src/lib/parser.ts", name: "parseChapterMark", sample: "【章】第 3 章 完", ok: (o) => o === 3 },
  {
    head: "立绘",
    from: "server/acp-server.mjs",
    name: "parseExpressionLine",
    sample: "【立绘】薇拉|微笑",
    ok: (o) => (o as { variant?: string } | null)?.variant === "微笑",
  },
  {
    head: "新剧本",
    from: "server/acp-server.mjs",
    name: "parsePresetAddedLine",
    sample: "【新剧本】rift-mark",
    ok: (o) => (o as { id?: string } | null)?.id === "rift-mark",
  },
  {
    head: "树",
    from: "server/acp-server.mjs",
  ...
  },
  {
    ...
    name: "parseAudioLine",
    sample: "【曲】雨夜",
    ok: (o) => (o as { kind?: string } | null)?.kind === "曲",
  },
  {
    head: "环境",
    from: "server/acp-server.mjs",
    name: "parseAudioLine",
    sample: "【环境】旅店大堂",
    ok: (o) => (o as { kind?: string } | null)?.kind === "环境",
  },
  {
    head: "音效",
    from: "server/acp-server.mjs",
    name: "parseAudioLine",
    sample: "【音效】门响",
    ok: (o) => (o as { kind?: string } | null)?.kind === "音效",
  },
];
...
describe("① 协议头集合唯一真源：PROTOCOL_HEADS ↔ isProtocolLine ↔ 解析出口 ↔ SKILL 备忘 ↔ 文档", () => {
  ...
  it("isProtocolLine 的正则由 PROTOCOL_HEADS 构造（真源在 shared，没有第二份手写头表）", () => {
    ...
    expect(
      uses,
      `PROTOCOL_HEADS 的 join 在 src/lib/parser.ts 里出现 ${uses} 次（应为 1 次）：多出来的那份就是第二真源，两边一分叉就说不清哪个才对`,
    ).toBe(1);
    ...
    expect(
      parserSrc,
      `src/lib/parser.ts 里出现了 PROTOCOL_HEADS 的数组字面量：真源已在 shared/protocol.mjs，这里再写一份就是第二真源`,
    ).not.toContain("PROTOCOL_HEADS = [");
    ...
    if (!decl)
      throw new Error(
        `${sharedRel} 里找不到 \`export const PROTOCOL_HEADS = [...]\`：协议头集合没有真源（v1.7 起从 src/lib/parser.ts 搬来），无法与文档/解析出口比对`,
      );
    ...
    for (const head of HEADS) {
      expect(
        isProtocolLine(`【${head}】…`),
        `isProtocolLine("【${head}】…") 应为 true（【${head}】是真源里的协议头，过滤清单漏了它）`,
      ).toBe(true);
    }
    expect(
      isProtocolLine("【图鉴】不是协议行"),
      "非协议头「【图鉴】」被 isProtocolLine 认下了：正则失之过宽，会把正文误当协议行过滤掉",
    ).toBe(false);
  });
  ...
  it("SKILL.md「标记格式备忘」提到全部 9 个头", () => {
    ...
    if (anchor === -1)
      throw new Error(`${rel} 里找不到「标记格式备忘」小节：契约 lint 的锚点（引擎侧协议行清单）被改名或删掉了`);
  ...
  it("docs/ARCHITECTURE.md 逐字写出 PROTOCOL_HEADS 声明并提到全部 9 个头", () => {
    ...
    expect(
      missing,
      `${rel} 提到这些协议头（谁和谁不一致：文档 ↔ PROTOCOL_HEADS 真源）：缺 ${missing.map((h) => `【${h}】`).join("")}；真源 9 头是 ${headsText()}`,
    ).toEqual([]);
  });
});
...
/**
 * RULES ↔ SKILL.md 的**共有不变量锚点**（v1.13 补）。
 *
 * 为什么不是整段逐字比对：这两份是不同文体的独立文本——注入引擎的 RULES（server/protocol-lines.mjs）
 * 是每回合随会话下发的短规则，SKILL.md 是引擎按需读的长文档。要求两者逐字一致等于逼着两边互相迁就文体。
 * 但它们必须**同意同一批事实**：选项段格式、目录口径、缓存例外、重绘指令字面量。这张表就是那批事实的字面锚点。
 *
 * 背景（为什么补这条）：`AGENTS.md` 曾声称这里保护的是「server↔SKILL.md 双份 + lint 逐字比对」，
 * 而实际只比了 server ↔ docs/ARCHITECTURE.md（SKILL.md 里根本没有 RULES 原文）——改 RULES 只改两处就能全绿，
 * 引擎真正读的那份（SKILL）可以照旧漂着。现在两边都钉：改一处、另一处不跟，契约 lint 当场红。
 */
const RULES_SKILL_ANCHORS = [
  { literal: "**行动**", what: "选项段格式：正文后加粗一行 + 每行一个编号选项" },
  { literal: "世界纪律", what: "世界目录纪律（命名 + 存在）" },
  { literal: "state/worlds/", what: "state 文件的实际落点" },
  { literal: "presets/<剧本 id>/assets/", what: "美术资产落点（资产随故事走）" },
  { literal: "presets/<剧本 id>/audio/", what: "音频资产落点（音频随故事走）" },
  { literal: "唯一例外", what: "生成前缓存检查的唯一例外（重绘）" },
  { literal: "美术：重绘", what: "重绘指令的字面量" },
];
...
describe("② RULES 逐字副本：server 常量 ↔ ARCHITECTURE 的 RULES 代码块", () => {
  it("server 的 RULES 是 6 句（RULES_SENTENCES），拼接串与它逐字相等，第 6 句是音频纪律句", async () => {
    ...
    expect(
      joined,
      '注入 agent 的 RULES 不再是 RULES_SENTENCES.join("")：两个视图分叉后，lint 比对的就不再是真正注入给引擎的那条串',
    ).toBe(sentences.join(""));
    expect(
      sentences[5],
      `RULES 第 6 句不是音频纪律句：现在是「${sentences[5]}」，应以「音频纪律：」开头（v1.6 音频三行全靠它约束）`,
    ).toMatch(/^音频纪律：/);
    expect(
      sentences[0],
      `RULES 第 1 句不再是【行动】文本选项句：现在是「${sentences[0]}」，应含 **行动** 字样`,
    ).toContain("**行动**");
  });
  ...
  it("docs/ARCHITECTURE.md 的 RULES 代码块逐句包含这 6 句（逐字）", async () => {
    ...
    if (!block)
      throw new Error(
        `${rel} 里找不到 RULES 代码块（应有一段以「本会话运行在自定义游戏客户端下：…」开头的围栏块）——文档与 server 的 RULES 常量已经对不上`,
      );
  ...
  it("RULES 与 SKILL.md 同意同一批事实（共有不变量锚点，两边都要有）", async () => {
    const rel = ".grok/skills/bunkiten/SKILL.md";
    const skill = read(rel);
    const { sentences } = await rulesOrThrow();
    const rulesText = sentences.join("\n");
    for (const { literal, what } of RULES_SKILL_ANCHORS) {
      expect(
        rulesText.includes(literal),
        `注入引擎的 RULES 里找不到锚点「${literal}」（${what}）：RULES_SENTENCES 改口径时把这个不变量弄丢了——它是引擎每回合都会读到的短规则`,
      ).toBe(true);
      expect(
        skill.includes(literal),
        `${rel} 里找不到锚点「${literal}」（${what}）：SKILL 是引擎按需读的长文档，与 RULES 是两份独立文本，` +
          `但必须同意同一批事实——改了一边没改另一边，引擎读到的两份规则就自相矛盾了`,
      ).toBe(true);
    }
  });
});
...
describe("③ 指令字符串双处存在：src/lib/parser.ts 源码 ↔ SKILL.md", () => {
  ...
  it("重绘可带玩家要求（v1.12）：`美术：重绘 …：<要求>` 的两端与长度口径都有（客户端构造 ↔ SKILL 规则）", () => {
    ...
    expect(parserSrc, "parser.ts 里没有 `美术：重绘 <类型> <名>：<要求>` 的模板").toContain(
      "美术：重绘 ${type} ${key}：${clean}",
    );
...
describe("⑦ 引擎凭据：服务目录 ↔ 设置屏 / MCP 工具名 ↔ SKILL / 凭据落点 ↔ 文档", () => {
  ...
  it("服务目录是唯一真源：PROVIDER_IDS 由 PROVIDERS 派生，GUI 吃真源而不是自带第二份 id 表", () => {
    ...
    expect(gui, `${GUI_PROVIDERS_FILE} 没有从 ${rel} import 服务目录：下拉会退化成另一份手写清单`).toContain(
      'from "../../shared/providers.mjs"',
    );
    expect(gui, `${GUI_PROVIDERS_FILE} 没有消费 providersFor（目录过滤在真源里，GUI 只渲染）`).toContain(
      "providersFor(",
    );
  ...
  it("设置屏的两组模式字面与 server 的 LLM_MODES/IMAGE_MODES 对得上（byok/session/off 三词不许各写各的）", () => {
    ...
    expect(creds, "server/credentials.mjs 里 LLM_MODES 不再是 [session, byok]").toMatch(
      /export const LLM_MODES = Object\.freeze\(\["session", "byok"\]\)/,
    );
    expect(creds, "server/credentials.mjs 里 IMAGE_MODES 不再是 [off, byok]").toMatch(
      /export const IMAGE_MODES = Object\.freeze\(\["off", "byok"\]\)/,
    );
    for (const mode of ['"session"', '"byok"', '"off"']) {
      expect(gui, `${GUI_PROVIDERS_FILE} 里没有模式字面量 ${mode}（设置屏的模式开关必须与服务端同一套词）`).toContain(
        mode,
      );
    }
  });
  ...
  it("MCP 出图工具名：media-mcp.mjs 的两个常量拼出 SKILL.md 里写的那把 catalog 键", async () => {
    ...
    expect(name, `server/media-mcp.mjs 的 MEDIA_MCP_NAME 变了：${name}（改名要同批改 SKILL 与本文档节）`).toBe(
      "bunkiten-media",
    );
    ...
    expect(src, "server/media-mcp.mjs 的 TOOL_DEFINITION 没有引用 MEDIA_TOOL_NAME：工具名有了第二份字面量").toContain(
  ...
  it("凭据落点与环境变量名：server 常量 ↔ 文档逐字一致（key 存哪、怎么注入引擎）", () => {
    ...
    const src = read(rel);
    expect(src, `${rel} 的 CREDENTIALS_DIRNAME 变了：${CREDENTIALS_DIRNAME}`).toContain(
      `export const CREDENTIALS_DIRNAME = "${CREDENTIALS_DIRNAME}"`,
    );
    expect(src, `${rel} 的 CREDENTIALS_FILENAME 变了：${CREDENTIALS_FILENAME}`).toContain(
      `export const CREDENTIALS_FILENAME = "${CREDENTIALS_FILENAME}"`,
    );
    ...
    for (const env of ENV_NAMES) {
      ...
      expect(doc, `${CREDENTIALS_DOC} 的「引擎凭据与自备 key」一节没有写 ${env}：引擎侧注入面没有文档锚点`).toContain(
        env,
      );
    }
    expect(doc, `${CREDENTIALS_DOC} 里没有两条出图途径的说明（MCP 工具与内置 image_gen 的优先级）`).toContain(
      "bunkiten-media__generate_image",
    );
  });
  ...
  it("发布源 docs/providers.json 与 shared/providers.mjs 的 PROVIDERS 深等（v1.10，ADR-0020；由 npm run providers:export 生成）", () => {
    ...
    expect(
      doc.version,
      `${rel} 的 version 变了：${String(doc.version)}（服务端只认 version 1，见 server/providers-catalog.mjs 的 CATALOG_VERSION）`,
    ).toBe(1);
    expect(
      doc.providers,
    ...
    expect(typeof doc.updatedAt, `${rel} 缺 updatedAt（发布时刻）：跑 \`npm run providers:export\` 会补上`).toBe(
      "string",
    );
  });
});
...
describe("⑧ 引擎后端：真源 ↔ 描述符表 / GUI / 文档 / 铺垫路径", () => {
  it("引擎真源是唯一一份：ENGINE_IDS 由 ENGINES 派生；描述符表与它同键（两边各加一个引擎而漏改另一边会红）", async () => {
    ...
    const src = read(rel);
    expect(src, `${rel} 的 ENGINE_IDS 不再由 ENGINES 派生：选择器与服务端校验会分成两份清单`).toMatch(
      /export const ENGINE_IDS = Object\.freeze\(ENGINES\.map\(/,
    );
    ...
    for (const e of SHARED_ENGINES) {
      if (!e.byok) {
        expect(
          e.byokNote.trim().length,
          `引擎「${e.id}」的 byok=false 却没给玩家话的原因（设置屏禁用按钮时要显示）`,
        ).toBeGreaterThan(0);
      }
    }
  });
  ...
  it("GUI 吃真源、不带第二份 id 表：设置屏与启动屏都 import shared/engines.mjs，src/ 下没有引擎 id 字面量", () => {
    ...
    expect(boot, `src/components/BootScreen.tsx 没有消费 engineById/loginHint（未登录文案按引擎分支）`).toContain(
      "engineById(",
    );
  ...
  it("打包铺场脚本与描述符算出同一个入口（跑 npm run dist:* 时铺出来的树要真能被 spawn）", () => {
    ...
    // 覆盖开关（测试/打包态用）与描述符同名
    expect(src, `server/engines.mjs 少了 BUNKITEN_CODEX_ACP 覆盖开关（harness 与打包冒烟靠它指到垫片）`).toContain(
      "BUNKITEN_CODEX_ACP",
    );
    expect(read(ENGINE_DOC), `${ENGINE_DOC} 没写 BUNKITEN_CODEX_ACP 覆盖开关（排障时找不到入口）`).toContain(
      "BUNKITEN_CODEX_ACP",
    );
    // grok 的模型覆盖旋钮（真引擎冒烟用便宜档跑）同理：源码与排障文档两处都要写
    expect(src, `server/engines.mjs 少了 BUNKITEN_GROK_MODEL 覆盖开关（真引擎冒烟靠它换便宜模型）`).toContain(
      "BUNKITEN_GROK_MODEL",
    );
    expect(read(ENGINE_DOC), `${ENGINE_DOC} 没写 BUNKITEN_GROK_MODEL 覆盖开关（排障时找不到入口）`).toContain(
      "BUNKITEN_GROK_MODEL",
    );
    // 主目录覆盖旋钮（打包态冒烟要把 home 指到临时目录）：真源在 config.mjs 的 gameHome()，文档也要写
    expect(
      read("server/config.mjs"),
      "server/config.mjs 少了 BUNKITEN_HOME 覆盖开关（打包态冒烟靠它换临时 home）",
    ).toContain("BUNKITEN_HOME");
    expect(read(ENGINE_DOC), `${ENGINE_DOC} 没写 BUNKITEN_HOME 覆盖开关（排障时找不到入口）`).toContain(
      "BUNKITEN_HOME",
    );
  });
});
...
/**
 * 用例数口径是**下限语义**：数字说的是「不少于」，不是精确值。
 *
 * 唯一维护点就是这个数组：文档只留一句粗口径（`单测 + 集成全量 N 例`，那个 N 同样是下限声明）。
 * 三档规则：
 *   · 加用例 —— 什么都不用改（实际 ≥ 下限自然成立）；
 *   · 删用例 —— 这里会红（防静默砍测试；确实要删就先在本文件压低对应 floor，一处）；
 *   · 抬高口径 —— 可选，只改这里的 floor 与 CASE_TOTAL（不抬高不会红）。
 *
 * v1.13 收紧：此前 floor 停在很久以前（合计 506，实际 655 ≈ 18% 冗余），
 * 「删掉一整个 describe」都不会红；且 engines / engine-login / packaged-app / declarations
 * 四个文件**根本没有分组**——整份文件被删也没人管。现在：
 *   · 每个测试文件都在某个分组里（唯一例外是 contract 自身，见下）；
 *   · floor 取「当前实际 − 2%」（每个文件都留一点点余量，而不是全局吃一个大缓冲）。
 *
 * ⚠️ 本门禁的能力边界（别高估它）：计数是**文本扫描**（见 countCases），不是 vitest 的收集口径——
 *   · 用循环生成用例的地方只数得到那个 `it(`（tests/declarations.test.ts 一行循环 = 运行时 3 例，文本 1 例）；
 *   · 它拦的是「有人整批删测试」，不是「某条断言被改弱」——后者靠各文件自己的断言。
 */
const CASE_GROUPS = [
  { name: "parser", files: ["tests/parser.test.ts"], floor: 68 },
  { name: "server", files: ["tests/server.test.ts"], floor: 117 },
  { name: "crafting", files: ["tests/crafting.test.ts"], floor: 61 },
  { name: "treeLayout", files: ["tests/treeLayout.test.ts"], floor: 6 },
  { name: "genealogy", files: ["tests/genealogy.test.ts"], floor: 7 },
  { name: "diff", files: ["tests/diff.test.ts"], floor: 7 },
  ...
  { name: "ui", files: ["tests/ui.test.tsx"], floor: 174 },
  {
    ...
    files: [
      ...
      "tests/integration/audio-history.test.ts",
      "tests/integration/http-guard.test.ts",
    ],
    floor: 29,
  },
  { name: "credentials-integration", files: ["tests/integration/credentials.test.ts"], floor: 21 },
  // 服务目录更新通道（v1.10，ADR-0020）：纯函数层 + 集成层两个文件并成一组
  {
    name: "providers-catalog",
    files: ["tests/providers-catalog.test.ts", "tests/integration/providers-catalog.test.ts"],
    floor: 36,
  },
  // v1.13 补进来的四组：此前它们不在任何分组里，整份文件被删都不会红
  { name: "engines", files: ["tests/engines.test.ts"], floor: 23 },
  { name: "engine-login", files: ["tests/engine-login.test.ts"], floor: 4 },
  { name: "packaged-app", files: ["tests/packaged-app.test.ts"], floor: 7 },
  // declarations 是循环生成用例（3 例）：文本只数得到 1 个 it(，floor 就按 1 钉——它守的是「文件还在」
  { name: "declarations", files: ["tests/declarations.test.ts"], floor: 1 },
  // 同样漏在门禁外的四个（v1.13 由「每个文件都要有分组」那条断言当场点出来）：
  // 小文件的 2% 余量没有意义（3 例的 2% 四舍五入是 0），一律 floor = 当前实际
  { name: "engine-auth", files: ["tests/integration/engine-auth.test.ts"], floor: 3 },
...
const CASE_SUB_GROUPS = [
  { name: "pipeline", file: "tests/integration/pipeline.test.ts", floor: 14 },
  { name: "audio-history", file: "tests/integration/audio-history.test.ts", floor: 10 },
 ...
 * v1.13 先剥掉整行注释再数：注释里写用例字面量不该进口径（历史文件里没有这种情况，把口径钉死免得日后有）；
 * 顺带写明它的**已知盲区**——循环里生成的用例只数得到那个 `it(`（见 CASE_GROUPS 的说明）。
 */
function countCases(rel: string): number {
  const src = read(rel).replace(/^\s*\/\/.*$/gm, "");
  return (src.match(/\b(it|test)\(/g) ?? []).length;
}

/**
 * `npm test` 实际会跑的全部测试文件（相对路径）。
 * e2e 四套（`tests/e2e…` 下的目录）是 Playwright 的，不在 vitest 口径里，按目录名前缀跳过。
 */
function listVitestFiles(dir = "tests"): string[] {
  const out: string[] = [];
  for (const entry of readdirSync(path.join(ROOT, dir), { withFileTypes: true })) {
    const rel = `${dir}/${entry.name}`;
    if (entry.isDirectory()) {
      if (entry.name.startsWith("e2e")) continue; // Playwright 的，不进 vitest 口径
      out.push(...listVitestFiles(rel));
...
describe("④ 用例数下限：CASE_GROUPS 的 floor ↔ 各文件实际用例数（唯一维护点）", () => {
  ...
  it("integration 三个子分组不低于各自下限，且下限自洽（子项之和 ≤ 分组 ≤ 合计）", () => {
    for (const sub of CASE_SUB_GROUPS) {
      ...
      expect(
        actual,
        `integration 子分组「${sub.name}」跌破下限：${sub.file} 现在实际 ${actual} 例、下限 ${sub.floor} 例`,
      ).toBeGreaterThanOrEqual(sub.floor);
    }
  ...
  it("实际合计不低于 CASE_TOTAL，且本文件不计入其中", () => {
    ...
    expect(self, `本文件 ${CONTRACT_FILE} 一个用例都没数到（${self} 例）：契约 lint 空跑等于没有门禁`).toBeGreaterThan(
      0,
    );
    for (const group of CASE_GROUPS) {
      expect(
  ...
  it("每个 vitest 测试文件都在某个分组里（新增文件不能落在门禁之外）", () => {
    const governed = new Set([...CASE_GROUPS.flatMap((g) => g.files), CONTRACT_FILE]);
    const orphans = listVitestFiles().filter((f) => !governed.has(f));
    expect(
      orphans,
      `这些测试文件不在 CASE_GROUPS 的任何分组里：${orphans.join("、")}——` +
        `后果是整份文件被删都不会红（v1.13 之前 engines/engine-login/packaged-app/declarations 就是这么漏的）。` +
        `新加测试文件时，顺手在本文件给它一个分组与 floor。`,
    ).toEqual([]);
  });
  ...
  it("三份文档不再逐分组钉数字（撤掉的税不要长回来；要查数字看 CASE_GROUPS）", () => {
    for (const doc of DOCS) {
      const text = read(doc);
      const offenders = DOC_GROUP_RES.flatMap((re) =>
        [...text.matchAll(re)].map((m) => `${at(doc, text, m.index)}「${m[0].replace(/\s+/g, " ").trim()}」`),
      );
      expect(
        offenders,
...
describe("⑤ 设置键与协议类型集合（音频/美术）：settings.ts / shared 真源 / 文档一致", () => {
  it("设置键 bunkiten.settings.v1 在 settings.ts 与文档里一致", () => {
    expect(
      ...
      `src/lib/settings.ts 的 SETTINGS_STORAGE_KEY 是「${SETTINGS_STORAGE_KEY}」，契约键是「${SETTINGS_KEY}」`,
    ).toBe(SETTINGS_KEY);
    for (const doc of ["docs/ARCHITECTURE.md", "README.md"]) {
      expect(
        read(doc),
        `${doc} 里没有设置键「${SETTINGS_KEY}」（src/lib/settings.ts 用的就是它：${SETTINGS_STORAGE_KEY}）`,
      ).toContain(SETTINGS_KEY);
    }
  });
  ...
  it("音频扩展名：shared 的 AUDIO_EXTS / AUDIO_MIME / 直服白名单同集，且文档逐字写全", () => {
    ...
    if (!decl)
      throw new Error(
        `${rel} 里找不到 \`const AUDIO_EXTS = [...]\`：音频扩展名没有真源（v1.7 起从 server/acp-server.mjs 搬来），无法与文档/客户端比对`,
      );
    const sharedExts = [...decl[1].matchAll(/"([^"]+)"/g)].map((m) => m[1]);
    expect(
      sharedExts,
      `${rel} 的 AUDIO_EXTS 与契约集合不一致：现在是 [${sharedExts.join(", ")}]，契约是 [${EXTS.join(", ")}]`,
    ).toEqual(EXTS);
    ...
    expect(
      mimeKeys,
      `${rel} 的 AUDIO_MIME 键与 AUDIO_EXTS 不同集：MIME 表 [${mimeKeys.join(", ")}]、扩展名 [${sharedExts.join(", ")}]`,
    ).toEqual([...sharedExts].sort());
    // 白名单/文件名正则都由真源构造（不是手写第二份）
    expect(shared, `${rel} 的 AUDIO_REL_RE 不再由 AUDIO_EXTS 构造：直服白名单会与扩展名真源脱钩`).toMatch(
      /const AUDIO_REL_RE = new RegExp\(`[^`]*\$\{AUDIO_EXTS\.join\("\|"\)\}/,
    );
    const fileRe = /const AUDIO_FILE_RE = new RegExp\(`[^`]*`\)/.exec(shared)?.[0] ?? "";
    expect(fileRe, `${rel} 的 AUDIO_FILE_RE 不再由 AUDIO_KINDS 构造：现在是「${fileRe}」`).toContain(
      '${AUDIO_KINDS.join("|")}',
    );
    expect(fileRe, `${rel} 的 AUDIO_FILE_RE 不再由 AUDIO_EXTS 构造：现在是「${fileRe}」`).toContain(
      '${AUDIO_EXTS.join("|")}',
    );
    ...
    expect(
      read(audioRel),
      `${audioRel} 应从 shared/protocol.mjs import 音频真源（AUDIO_KINDS/EXTS/FILE_RE）：找不到对应的 import 语句`,
    ).toMatch(/import \{[^}]*\bAUDIO_(?:FILE_RE|KINDS|EXTS)\b[^}]*\} from "\.\.\/shared\/protocol\.mjs"/);
    // 不得再自持第二份扩展名/MIME 声明——v1.7 拆模块后 server/ 有 10 个文件，防第二真源的扫描必须覆盖全目录而不只入口
    for (const file of serverSources()) {
      expect(file.src, `${file.rel} 里出现了第二份 AUDIO_EXTS 声明：真源已在 ${rel}`).not.toMatch(
        /const AUDIO_EXTS = \[/,
      );
      expect(file.src, `${file.rel} 里出现了第二份 AUDIO_MIME 声明：真源已在 ${rel}`).not.toMatch(
        /const AUDIO_MIME = \{/,
      );
    }
  ...
  it("音频类型集合（曲/环境/音效）shared 与 parser 同序同字面", () => {
    ...
    if (!decl)
      throw new Error(
        `${rel} 里找不到 \`const AUDIO_KINDS = [...]\`：音频类型字面没有真源（v1.7 起从 server/acp-server.mjs 搬来）`,
      );
    const sharedKinds = [...decl[1].matchAll(/"([^"]+)"/g)].map((m) => m[1]);
    expect(
      sharedKinds,
      `${rel} 的 AUDIO_KINDS 与 src/lib/parser.ts 的 AUDIO_KINDS 不一致：shared [${sharedKinds.join("、")}]、parser [${[...AUDIO_KINDS].join("、")}]（parser 应 re-export 真源，分叉即第二真源）`,
    ).toEqual([...AUDIO_KINDS]);
  });
  ...
  it("美术类型集合（立绘/背景/封面）与资产文件名正则：shared 真源、双侧构造、无第二份字面", async () => {
    ...
    const src = read(rel);
    const kinds =
      /const ART_KINDS = Object\.freeze\(\[([^\]]+)\]\)/.exec(src) ?? /const ART_KINDS = \[([^\]]+)\]/.exec(src);
    ...
    expect(src, `${rel} 的 ASSET_FILE_RE 不再由 ASSET_KINDS 构造：落盘/直服白名单会与类型真源脱钩`).toMatch(
      /const ASSET_FILE_RE = new RegExp\(`[^`]*\$\{ASSET_KINDS\.join\("\|"\)\}/,
    );
    ...
    for (const [file, marker] of [
      ...
      expect(read(file), `${file} 没有用 ${marker} 从 shared 真源构造类型交替组：手写字面会与真源分叉`).toContain(
        marker,
      );
    }
    ...
    expect(
      offenders,
      `这些文件手写了 立绘|背景 形态的类型交替组：类型真源在 ${rel} 的 ART_KINDS/ASSET_KINDS，落盘白名单走 ASSET_FILE_RE`,
    ).toEqual([]);
    ...
    for (const kind of ["立绘", "背景", "封面"]) {
      expect(
        rules,
        `RULES 的【图】句没有枚举美术类型「${kind}」：改 ${rel} 的 ART_KINDS 要同步 RULES 第 3 句（以及 SKILL.md【美术】）`,
      ).toContain(kind);
    }
  });
});
...
describe("⑥ 指令前缀、章标记与主题白名单：shared 真源 ↔ server 消费点；server ↔ theme.ts 同集", () => {
  it("DIRECTIVE_PREFIX_RE 单一真源：pickEffort 与 isMainTurn 都消费它，server 源码无第二份前缀字面", () => {
    ...
    for (const hit of [
      ...
      expect(SHARED_DIRECTIVE_RE.test(hit), `DIRECTIVE_PREFIX_RE 应命中「${hit}」：分档/快照判定漏了这个前缀`).toBe(
        true,
      );
    }
    for (const miss of ["推开门看看。", "开局：《雨巷》。", "继续世界：w1。"]) {
      expect(
        SHARED_DIRECTIVE_RE.test(miss),
  ...
  it("CHAPTER_MARK_RE 单一真源：parseChapterMark 消费它，server 质量守卫豁免也消费它，且无第二份章标记正则字面", () => {
    ...
    expect(
      SHARED_CHAPTER_RE.test("【章】第 3 章 完"),
      `CHAPTER_MARK_RE 应命中「【章】第 3 章 完」：章末回合豁免漏了正常形态就会误追问`,
    ).toBe(true);
    expect(
      SHARED_CHAPTER_RE.test("（终章正文）\n\n【章】第 3 章 完\n"),
      "CHAPTER_MARK_RE 应命中回合文本中间的章标记行（m 多行锚定）",
    ).toBe(true);
    expect(
      SHARED_CHAPTER_RE.test("【章】"),
      "CHAPTER_MARK_RE 不该命中单独的「【章】」：残缺行不是章末回合，失之过宽会把豁免当挡箭牌",
    ).toBe(false);
    expect(
      parseChapterMark("【章】第 3 章 完"),
      "parseChapterMark 对样例章标记应给章号 3（消费真源后公共 API 语义不变）",
    ).toBe(3);
    ...
    expect(
      parserSrc,
      `${parserRel} 应从 shared/protocol.mjs import CHAPTER_MARK_RE（v1.7 起真源在那）：找不到对应的 import 语句`,
    ).toMatch(/import \{[^}]*CHAPTER_MARK_RE[^}]*\} from "\.\.\/\.\.\/shared\/protocol\.mjs"/);
  ...
  it("主题取值真源在 shared/theme.mjs：两侧 import 它、没有第二份，global.css 初始值同批", async () => {
    const mod = (await import("../shared/theme.mjs")) as {
      MOTIFS: readonly string[];
      FONT_PRESETS: readonly string[];
      DIALOG_TEXTURES: readonly string[];
      FALLBACK_THEME: Record<string, string>;
    };
    // ① 真源的值与契约清单逐字（顺序也是契约：GUI 下拉与回退提示按它渲染）
    expect(
      [...mod.MOTIFS],
      `shared/theme.mjs 的 MOTIFS 变了：现在是 [${[...mod.MOTIFS].join(", ")}]——母题集变了要么是新增图案（motifs/ 也要加），要么是打字错了`,
    ).toEqual(["summer", "rune", "imperial", "aurora"]);
    expect(
      [...mod.FONT_PRESETS],
      `shared/theme.mjs 的 FONT_PRESETS 变了：现在是 [${[...mod.FONT_PRESETS].join(", ")}]（每加一档，src/theme.ts 的 FONT_STACKS 也要加对应字体栈）`,
    ).toEqual(["serif", "song", "kai", "hei"]);
    expect(
      [...mod.DIALOG_TEXTURES],
      `shared/theme.mjs 的 DIALOG_TEXTURES 变了：现在是 [${[...mod.DIALOG_TEXTURES].join(", ")}]（每加一档，global.css 要有对应的 .dialog-* 规则）`,
    ).toEqual(["plain", "silk", "paper", "glass"]);
    expect(
      { ...mod.FALLBACK_THEME },
      "shared/theme.mjs 的兜底主题变了：它同时是 /api/presets 的填值与客户端的回退值，改它要同批改 global.css 的初始变量",
    ).toEqual({ accent: "#c9a86a", accent2: "#e8e4da", motif: "aurora", font: "serif", dialog: "plain" });

    // ② 两侧必须**import 真源**，而不是各自再写一份（这是「没有第二份」的正面证据）
    for (const [file, why] of [
      ["server/presets.mjs", "server 侧 normalizeTheme 的逐键兜底真值"],
      ["src/theme.ts", "客户端 getTheme/dialogClass 的兜底"],
    ]) {
      expect(
        read(file),
        `${file} 没有从 shared/theme.mjs import 主题取值（${why}）——v1.13 起真源在那里，自己再写一份就是第四处拷贝`,
      ).toMatch(/import \{[^}]*FALLBACK_THEME[^}]*\} from "\.\.(\/shared\/theme\.mjs|shared\/theme\.mjs)"/);
    }

    // ③ 第二份**声明**扫描：server/ · src/ · scripts/ 里都不许再声明这些常量（re-export 不算声明）
    const declared = ["FONT_PRESETS", "DIALOG_TEXTURES", "MOTIFS", "FALLBACK_THEME", "DEFAULT_THEME"];
    const sources = [
      ...serverSources(),
      ...srcFiles().map((rel2) => ({ rel: rel2, src: read(rel2) })),
      ...readdirSync(path.join(ROOT, "scripts"))
        .filter((f) => f.endsWith(".mjs"))
        .map((f) => ({ rel: `scripts/${f}`, src: read(`scripts/${f}`) })),
    ];
    for (const file of sources) {
      for (const name of declared) {
        const copies = (file.src.match(new RegExp(`const ${name}\\b`, "g")) ?? []).length;
        expect(
          copies,
          `${file.rel} 里声明了 ${name}（${copies} 处）：主题取值真源只有 shared/theme.mjs 一份，别处再声明就是第 N 份拷贝（要引用就 import）`,
        ).toBe(0);
      }
    }

    // ④ 那条被打破的环不许长回来：presets.mjs 再 import 入口，acp-server ↔ presets 又会成环
    expect(
      read("server/presets.mjs"),
      "server/presets.mjs 又 import 了入口 acp-server.mjs——那是 v1.13 打破的环（acp-server ↔ presets），别接回去",
    ).not.toMatch(/from "\.\/acp-server\.mjs"/);

    // ⑤ global.css 的 --accent/--accent2 初始值（首帧/无注入态）：CSS 没法 import，这一份只能靠 lint 钉
    const cssSrc = read("src/styles/global.css");
    for (const key of ["accent", "accent2"]) {
      expect(
        cssSrc,
        `src/styles/global.css 的 --${key} 初始值与兜底主题分叉：shared/theme.mjs 是 ${mod.FALLBACK_THEME[key]}，CSS 初始变量应写同值（改兜底色时真源 + CSS 两处同批改）`,
      ).toContain(`--${key}: ${mod.FALLBACK_THEME[key]};`);
    }
  });

  it(".mjs 里不许出现 TS 专属语法（`as` 断言 / `: 类型`）：tsc 放行、打包器解析不过", () => {
    // 为什么单列一条：`allowJs` 下 tsc 是**容忍** TS 语法的，但 vite/rolldown 按纯 JS 解析 .mjs——
    // 一个 `x as T` 能一路过 tsc、过单测，直到某条用例 import 到它才在打包层炸（v1.13 实测：
    // scripts/doctor.mjs 里一句 `(MOTIFS as readonly string[])` 让 doctor.test 与 server.test 双双解析失败）。
    // 这条门禁把「只有打包器才发现的错」提前到 `npm test`。
    const targets = [
      ...serverSources(),
      { rel: "scripts/doctor.mjs", src: read("scripts/doctor.mjs") },
      { rel: "scripts/export-providers.mjs", src: read("scripts/export-providers.mjs") },
      { rel: "scripts/stage-codex-acp.mjs", src: read("scripts/stage-codex-acp.mjs") },
    ];
    for (const file of targets) {
      // 只看 ` as ` 断言（`import { a as b }` 与字符串里的 "as" 都不该误伤：前者带大括号、后者在引号里）
      const hits = file.src
        .split("\n")
        .map((line, i) => ({ line, i: i + 1 }))
        .filter(
          ({ line }) =>
            /\sas\s+[A-Za-z_$[{]/.test(line) &&
            !/^\s*\/\//.test(line) &&
            !/import\s|\bfrom\s|\{[^}]*\bas\b[^}]*\}/.test(line),
        );
      expect(
        hits.map((h) => `${file.rel}:${h.i}`),
        `${file.rel} 里出现了 TS 的 \`as\` 断言（${hits.length} 处）：.mjs 里它过得了 tsc（allowJs 容忍），` +
          `但 vite/rolldown 按纯 JS 解析会当场报 "Type assertion expressions can only be used in TypeScript files"。` +
          `改用 JSDoc 断言：\`/** @type {T} */ (expr)\`。`,
      ).toEqual([]);
    }
  });
});

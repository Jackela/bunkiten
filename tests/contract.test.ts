...
/** 文档口径的分组（README / AGENTS / ARCHITECTURE 三处声明），合计 439 例；不含 e2e 与本文件 */
const CASE_GROUPS = [
...
/** 契约 lint 自己（也被 npm test 收录，但按文档口径**不计入** 439） */
const CONTRACT_FILE = "tests/contract.test.ts";

/** 文档声明的合计口径 */
const CASE_TOTAL = 439;
...
/** 合计声明（README「单测 + 集成全量 439 例」、AGENTS「**共 439 例**」） */
const DOC_TOTAL_RE = /(?:共|全量)\s*(?:\*\*)?(\d+)(?:\*\*)?\s*例/g;

/**
 * 「合计口径」的其它既有写法：这些数字同样必须等于 CASE_TOTAL（现在 439）。
 * 由来：AGENTS 的「自身不计入 439 口径」这类措辞在历次计数同步里都漏网过——数字换了、措辞没跟上，
 * 只能靠人眼才发现——这几条模式专门兜住这种漂移；文档换新措辞时把新模式补进来。
 */
const DOC_TOTAL_ALT_RES = [
  /不计入(?:这)?\s*(?:\*\*)?(\d+)/g, // 「不计入 439 口径」「不计入这 439**」
  /与\s*(\d+)\s*\/\s*contract lint/g, // 「与 439/contract lint 计数无关」
  /上述\s*(\d+)\s*例/g, // 「即上述 439 例 + 契约 lint」
];

...
> v1.10.1 —— 给单人定制的 LLM 互动 AVG（文字冒险 / 视觉小说）完整产品：Electron 桌面壳 + React 前端 + grok CLI（ACP）叙事引擎；本版给 v1.10.0 收尾——**服务目录更新窗口收窄**（两个发布源并行抓、按 `updatedAt` 取最新，TTL 24h → 6h，`/api/providers` 读路径 stale-while-revalidate：先回当前目录、后台刷新——「发布 → 玩家可见」从「最长约一天 + 必须重启」压到「约 5 分钟 + 6h + 一次打开设置」；离线或校验失败照旧回落内置表），**签名就绪（仍需你出手）**（Windows 侧撤掉 `win.signExecutable: false` 那道硬关、改成条件化：`WIN_CSC_LINK` 存在即签名 + 时间戳，缺则与今天完全一致；新增 `docs/RELEASING.md` 写清要买的证书、要加的 secrets、开启后会发生什么与怎么验一次发布——**本版仍是未签名产物**），依赖三项 minor 升级（framer-motion / lucide-react / electron）并把 jsdom 钉回 `~30.0.1`（30.1.0 让 6 条 UI 单测红，已加 dependabot ignore 与复访条件）。

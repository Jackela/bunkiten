...
> v1.10.0 —— 给单人定制的 LLM 互动 AVG（文字冒险 / 视觉小说）完整产品：Electron 桌面壳 + React 前端 + grok CLI（ACP）叙事引擎；本版把「引擎用什么模型、用什么出图」搬进设置屏——**自备引擎凭据**（对话 / 出图两节：服务目录 27 条 + 自定义、地址/密钥/模型、出图尺寸、真连一次的「测试连接」、保存即时生效与「立刻重启引擎」；凭据只落 `~/.bunkiten/credentials.json`（0700/0600、永不明文出口），**不登录 grok 也能玩**），**打包态出图修好**（`asarUnpack` 覆盖 `server/**` + `shared/**` 整树，打包冒烟新增真 MCP 握手断言——此前打包态出图静默不工作），**引擎 skill 注入**（spawn 固定带 `--plugin-dir <gameRoot>/.grok`，开发态与打包态开局都拿得到 bunkiten skill），**服务目录可在线更新**（仓库 JSON 发布源 + 24h 缓存 + 内置兜底，离线自动回落、永不改写你已存的地址与密钥），另有出图工具名容错与启动链超时兜底。

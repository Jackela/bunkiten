...
| 登录检查 | 浏览器访问 5173 时 boot 屏同样检查 `/api/auth` | 同左；未登录时 boot 屏**按当前引擎**给指引（grok → `grok login`；codex → 随包 CLI 的 `login`），启动屏与设置屏共用 `lib/engine-login.ts` 的登录等待轮询 |

端口：默认 7800，被占则自动 +1（最多重试 10 次，即 7800–7810）；vite 代理固定指向 7800。

环境变量：`GROK_GAME_ROOT`（游戏数据根）、`PORT`（起始端口）、`EFFORT`（推理档，默认 `medium`；真源读在 `server/config.mjs`，入口只 re-export）、`ACP_PROXY_TARGET`（仅 dev 代理目标）、`BUNKITEN_HOME`（覆盖主目录——测试与打包态冒烟把 home 指到临时目录；**Windows 上不能改 `USERPROFILE`**，Chromium 读它）。
...
| `session/new` | `{ cwd: GAME_ROOT, mcpServers, _meta }` | `session/load` 失败或无存档时降级新建。**`mcpServers` 按需注入**（v1.10，ADR-0019）：只有 `image.mode === "byok"` 时才挂自建的 `bunkiten-media`，否则是空数组——见「引擎凭据与自备 key」 |
...
SSE 事件全集（`/events`）：`turn_start` / `seg{seg,label}` / `chunk{seg,text}` / `expression{character,variant}` / `presetAdded{id}` / `treeEdited` / `audio{kind,name}` / `turn_end{seq}` / `error{message}`。`turn_end` 的 `seq` 是**本回合的快照序号**（v1.13；三文件全等被去重时给当前最新 seq，非正戏回合为 null）——客户端拿它当玩家看到的「第 N 幕」，与存档点标注/回溯分割线同基底。`expression`/`presetAdded`/`treeEdited`/`audio` 由【立绘】/【新剧本】/【树】/【曲】【环境】【音效】协议行派发（只转发、不落盘）。`turn_end` 后 server 补跑一次未落盘资产的持久化（图片文件可能晚于标记到达），并在 `flushArtLines()` 之后、`busy=false` 之前落一条逐轮快照（见「逐轮状态快照与精确回退」）。

## 文本协议契约（最重要）

引擎输出的是纯文本，客户端靠字符串约定驱动 UI。**以下每一条字符串都是跨进程契约，改动必须五处同步**（`SKILL.md` / `parser.ts` / `turn-pipeline.mjs` / `store` / `tests` 快照，见「修改指引」）；协议常量（协议头集合、音频白名单与直服正则、指令前缀正则）的**唯一真源是 `shared/protocol.mjs`**（v1.7：`src/lib/parser.ts` re-export、`server/turn-pipeline.mjs` import 同一份值——此前双侧各一份、只靠 lint 比对两份源码文本，见 `docs/adr/0012`）。机器门禁是**契约 lint**（`tests/contract.test.ts`）断言八组契约：`RULES` 逐字副本（server 常量 ↔ 本节代码块——引擎只读 `.grok/` 提示词、不会 import 代码，这份天然双份）、`PROTOCOL_HEADS` 集合（shared 真源 ↔ server/parser 解析出口 ↔ SKILL 备忘 ↔ 本文档）、指令字符串双处存在（`parser.ts` ↔ `SKILL.md`）、指令前缀正则 `DIRECTIVE_PREFIX_RE` 单一真源（`pickEffort` 与 `isMainTurn` 都消费它）、主题白名单与兜底主题（server ↔ `src/theme.ts` 同集）、用例数下限（三份文档各一句「共/全量 N 例」的**粗口径**声明：N ≥ 该文件的 `CASE_TOTAL` 且 ≤ 实际；分组数字的唯一维护点是 `CASE_GROUPS`，文档**禁止**写逐分组数字）、设置键与音频扩展名三处一致、引擎凭据（⑦ 组：服务目录 ↔ GUI、MCP 工具名 ↔ SKILL、凭据落点与 env 名 ↔ 文档）与引擎后端（⑧ 组：引擎真源 ↔ 描述符表 ↔ GUI ↔ 文档）。改了契约字符串，这几处断言必须同批改（只改一处会被 lint 挡在测试里）。**该 lint 自身不计入文档里声明的合计口径**。
...
**默认路径不再等整章画完才开演**：规划回合拿到制作清单后，客户端会读本世界的剧情树、取「带进度指针的那个
节点的地点与在场」，把清单分成两段——**开场子集**（该地点的背景 + 在场角色的**基础**立绘；差分不进子集，
首现用不到、缺了还有两级回退）与**延迟队列**（其余项）。子集画完立刻发「开演。」，玩家先玩；延迟队列由
`pumpDeferredArt` 在**引擎空闲的空隙**逐张补（每轮空闲只发一项，补画分摊在玩家读正文的时间里）。

四条纪律（改这条链先看这里）：

- **判不了就退回全量**：没有世界 id、拉不到树（404/网络）、没有指针节点、地点与在场都空、或一个都没匹配上
  （用既有的 `assetNameMatches` 做 trim + 互为包含的名字对齐）——一律按老流程画完全部。**不做猜测**：猜错的
  代价是开演那一刻引擎现场生图，把等待从「开演前」挪到「开演后」，比慢更糟。依据是 SKILL「开演。」的语义
  ——「从当前节点演出（每章首次即树的第一个节点）」与子集严格对应。
- **补画回合是指令回合**：`artAsk` 置位期间，正文不进历史/对话（与 `treeAsk` 同款），【图】标记**只点槽位、
  不换画面**（`applyMarkers` 的 `silent` 档——否则一次补画会把玩家正在看的背景/立绘换掉）。
- **玩家输入优先**：补画进行中玩家的点选/输入进 `pendingPlayerPrompt`（单槽、后到覆盖）而不是去撞 409；
  回合收尾先补发它、再继续补画。输入闸只看 `artAsk`（补画指令发出到 `turn_start` 抵达之间那段网络窗口
  里 `engineBusy` 仍是 false，只看它会漏拦）。补画回合失败由 `onEngineError` 清标记并把在跑的那项记失败。
- **队列不持久化、跨章保留、换局清零**：`deferredArt` 是内存态；跨章保留（下一章的清单只列本章新增项，
  不保留就永久丢），换世界线/换剧本/重开由 `resetRunState` 清空。真丢了也有兜底——引擎在正戏里按需生成
  （v1.2 起就有的懒加载通路），只是那一回合会阻塞在出图上。
...
屏幕流（zustand `screen` 状态机）：`boot`（登录检查）→ `title`（卡带式剧本轮播：`← →` 切卡、Enter / 点中央卡「插卡」装载，卡底铺各自封面、无封面回退主题渐变；右下角落簇 v1.12 起是 素材 / 创作新剧本 / 设置 三枚可见 + 「更多 ▾」菜单（导出当前卡 / 导入剧本 / 剧本体检）——导出/导入见「剧本导出包」）→ `worlds`（世界线屏：列出该剧本已有世界，继续/新世界线/两段确认删除/改名/导出/导入；键盘导航）→ `protagonist`（捏人 chips / 快速开局）→ `crafting`（仅「制作美术并开演」路径：待命开局 → 章节规划 → 逐项美术指令 → `开演。`，见「分项美术指令」「章节与剧情树」）→ `game`。章间循环：game 屏收到【章】标记 → 切回 crafting（「第 N+1 章 · 制作中」）→ 自动规划下一章 → 制作 → 开演，往复直至终局。overlay 屏 `assets`（画廊）、`creation`（创作模式）、`tree`（剧情图）与 `settings`（设置）从 title/game/世界线屏进入，`closeOverlay` 返回进入前的原屏（不动回合与画面状态，画廊重绘与游戏态共用引擎回合）；Esc 关闭链（`App.tsx`，输入框聚焦时不拦截——`type="range"` 的滑杆除外，设置屏滑杆聚焦时 Esc 仍关屏）从里往外为「画廊大图预览 → 剧情图节点详情 → 历史抽屉 → 角色面板 → 创作屏退出确认 → 剧情图屏 / 设置屏 → 世界线屏（回到标题屏）」。
...
| `components/CraftingScreen.tsx` | 制作中屏（规划 → 逐项美术 → 开演）：进度行给「美术 N/M 就绪 · 平均 ≈Ns/张 · 约还需 ~X」（v1.13 可预期性；平均 = （现在 − `preloadBatchStartedAt`）/ 已完成张数，样本不足 2 张不给数，口径在导出的纯函数 `preloadEtaLabel`），另有一句「跳过剩余会立刻开演，未就绪的美术可从画廊补画」 |
| `components/SettingsScreen.tsx` | 设置 overlay（三处入口：命令轨「设置」/ 标题屏角落簇 / 启动屏未登录态的「打开设置」）：**第一层可见**——音频「主音量 / 静音 / 曲 / 环境 / 音效」+ 文本「文字速度四档 / 自动前进三档」；**第二层默认收起**（v1.12 两层披露）——「细分音量」与「引擎与密钥」（v1.10 整节：引擎选择 + 对话/出图两组自备密钥 + 测试连接 + 立刻重启引擎；从启动屏未登录态进来时自动展开）。每个控件直接调 `updateSettings`（无「保存」按钮，改动即时生效：AudioManager 立刻生效 + 写 localStorage），关屏走 `closeOverlay` |
...
| `lib/useCanvasPanZoom.ts` + `components/CanvasZoomToolbar.tsx` | 两块 SVG 画布（剧情图的 `TreeCanvas` 与家谱的 `GenealogyCanvas`）共用的 React 外壳（v1.13）：视图 state、布局变化回「适应」、非 passive 滚轮（指针锚点）、拖拽死区与指针捕获、`+ - 0` 快捷键、屏幕像素→布局坐标换算，以及「工具条三枚缩放控件 + 读数」（testid 前缀 `tree`/`genealogy`）。几何数学仍在 `lib/treeLayout.ts`；roving tabIndex、方向键走位、节点/边渲染、家谱的渲染宽度上界盒都留在各自组件里 |
...
| `theme.ts` | 剧本主题：`getTheme` 逐键校验兜底（默认 aurora + serif + plain）、`themeVars` 注入 `--accent`/`--accent2`/`--font-preset`、`dialogClass` 映射对话框质感类、`FONT_STACKS` 系统字体栈（离线无 webfont）；**白名单与兜底值 v1.13 起 import 自唯一真源 `shared/theme.mjs`**（`FONT_PRESETS`/`DIALOG_TEXTURES`/`MOTIFS`/`FALLBACK_THEME`，本文件不再抄第二份），只留客户端特有的 6 位 hex 校验 `HEX_RE` 与字体栈/质感类映射 |
| `lib/replay.ts` | 存档点重演的「退到哪」纯函数（v1.13，ADR-0023）：`prevTurnSnapshotSeq(snapshots, seq)`（seq 之前最近的一条 `kind:"turn"` 条目 = 该幕开演前的状态，backup 不计）与 `canReplaySnapshot(snapshots, seq)`（树屏重演入口的出现条件）。store 的解析内核与剧情图屏共用这一份规则——退到 K 自己会把同一幕演两遍、上下文还接错 |
| `lib/status.ts` | 引擎状态文案 → 玩家说法的唯一映射：`playerStatus(status)`（「引擎演绎中…」→「故事展开中…」、「出错：X」→「出错了：X」，表外原样透传），顶栏 / 制作中屏 / `StatusAnnouncer`（aria-live）三处共用——store 里的原字符串一个字不动 |
| `lib/menuTab.ts` | 菜单内 Tab 走位纯函数（v1.12）：Radix 菜单项是 `tabIndex=-1` 且弹层吞掉 Tab，`tabThroughMenu` 走到两端就把 Tab 交还页面（焦点搬到文档序里弹层之后/之前的第一个可聚焦元素，菜单随之被 focusOutside 收掉；**刻意不回绕**，WCAG 2.1.2）；世界线行的 `⋯` 菜单与命令轨的分组菜单共用（纯 DOM、不 import React，可直测） |
| `lib/worlds.ts` | 世界线显示层兜底：`isLegacyForkNote` 把旧版 server 写进索引的分叉备注判为「没有备注」（`note` 会被当世界名显示，等于把裸 id 摊给玩家） |
| `lib/engine-login.ts` | `watchLogin`（v1.11）：启动屏与设置屏共用的登录等待轮询（在途登录单例 + 输出尾巴 + 收尾），浏览器里登录完成即自动继续 |
| `components/ShellPage.tsx` | 壳层页框（v1.8）：满幅主题底 + 全仓唯一一处定宽 `max-w-[84rem]` + 统一表头（eyebrow → 标题 + 短 accent 分隔线 → 右侧动作簇）+ 可选 ≥xl 360px 右栏 + 可选页脚提示行；**6 屏**共用（worlds/protagonist/crafting/settings/assets/preset-check），壳层屏不再各屏自写居中窄栏 |
| `components/EngineKeysSection.tsx` | 「引擎与密钥」设置节（v1.10，ADR-0019）：v1.13 拆成三块——**容器**（引擎选择器 `engine-backend-<id>` + 登录状态行 + 「立刻重启引擎」= `POST /api/engine/restart` + 在线目录标注 `engine-catalog-online`，三处取数收进 `lib/useAsync.ts`）、`components/EngineGroupForm.tsx`（**一组**凭据的表单，对话/出图共用：目录下拉（在线目录优先、`providersFor` 回落）+ 地址/密钥/模型 + 出图两格尺寸；「测试连接」三态；文本 400ms 防抖保存、下拉与模式立即发；key 失焦提交并回掩码）、`components/EngineAuthPanel.tsx`（登录状态 + 一键登录 / 两段确认登出）。文案是玩家话（服务地址/密钥/模型），不出现 env 与变量名 |
| `components/game/ChapterCard.tsx` | 章节过场卡（v1.8）：章号一变在画面正中亮一次「第 N 章」，`CHAPTER_CARD_MS` 2200ms 在屏 + 末尾 400ms 淡出（两常量导出给用例当时间真源）；挂在 `App` 常驻层（不在 GameStage 里，跨屏记忆放组件内 ref），`pointer-events-none`、无可聚焦元素 |
...
| `lib/useAsync.ts` | 一次异步取数的通用形态（v1.13）：`useAsync(load, key)` 回 `{data,loading,error,reload}`，把散在各屏手抄的 `AbortController`/loading/error/abort effect 收成一处——**写回前复查 `signal.aborted`**，过期应答不落到已卸载/已换 key 的屏上；`load` 依赖拼进 `key`（不进 effect deps），`key === null` 表不取数 |
| `lib/tree-view.ts` | 剧情图屏展示层纯函数（v1.13 从 StoryTreeScreen 收编，不 import React/DOM）：`chapterNoOf`/`chapterLabel`（章号取节点 id 的 `N-M` 前缀）、`chapterItems`（章节条目 + 稳定键——章号唯一即章号、重号退回 `章号-下标`）、`STATUS_SLUG`（节点状态 → ASCII slug）、`archiveKey`（归档药丸键）、`snapshotTurnNo`、`earliestSnapshotByNode`、对比基线 `prevSnapshotSeq`；⚠️ 与 `lib/replay.ts` 的 `prevTurnSnapshotSeq` **同名不同义**（对比基线 kind 不限 vs 重演回退点只认 turn），别合并 |
| `components/TreeDetail.tsx` | 剧情图节点详情面板（v1.13 从 StoryTreeScreen 抽出）：地点/在场/梗概/出边/状态 + 存档点标注与命名（≤40 字）+ 在此分叉 / 回退到此节点 / 回到这一幕并重演（各两段确认）+ 内嵌三 tab diff（`partitionDiff` 折叠 ±2 行上下文）；自带取数（`fetchSnapshot`）与换节点复位 |
| `components/WorldRow.tsx` | 世界线列表的一行（v1.13 从 WorldsScreen 抽出）：缩略图（复用剧本封面 `coverUrl`）+ 显示名/备注 + 主行动「继续」+ `⋯` 菜单（改名/导出 `.world.json`/删除，删除的两段确认收在菜单内）；行是 listbox 的 `role="option"`，「继续」与菜单是它的兄弟（a11y：option 里不套交互控件） |
| `components/GenealogyCanvas.tsx` | 家谱画布（v1.13 从 WorldsScreen 抽出）：forkedFrom 森林 SVG；视图能力（滚轮锚点缩放/拖拽平移/双击复位/`+ - 0`）复用 `lib/useCanvasPanZoom`，与剧情图 `TreeCanvas` 同款；渲染宽度**只封上界**（`GEN_MAX_NODE_PX = 270`） |
| `components/EngineGroupForm.tsx` | 「引擎与密钥」里**一组**凭据的表单（v1.13 从 EngineKeysSection 抽出，对话/出图共用）：目录下拉（在线目录优先、`providersFor` 回落）+ 地址/密钥/模型 + 出图两格尺寸；文本 400ms 防抖保存、下拉与模式立即发；key 失焦提交并回掩码（本地明文只在输入期间存在） |
| `components/EngineAuthPanel.tsx` | 「引擎与密钥」里的登录状态 + 登录/登出块（v1.13 从 EngineKeysSection 抽出）：状态行（已登录/未登录/读取中）+ 一键登录（浏览器完成后经 `watchLogin` 自动刷新）+ 两段确认登出；状态与动作仍留在 EngineKeysSection |
...
- **壳层页框 `components/ShellPage.tsx`**：worlds / protagonist / crafting / settings / assets / preset-check 这 6 屏共用的外框（title 是卡带舞台、creation/story-tree 各有版式、boot 是首启三态，都不走它）——满幅主题底 + 宽栏 + 统一表头（eyebrow → 标题 + 短 accent 分隔线 → 右侧动作簇）+ 可选 ≥xl 右栏 + 可选页脚提示行。它约束的规则是：**壳层屏不再各屏自写居中窄栏**（旧屏的 `mx-auto max-w-3xl` 那类）。整屏只有 `ShellPage` 这一处定宽（`max-w-[84rem]`），屏内区块一律撑满可用宽度、靠栅格与多列组织信息；右栏只在 ≥xl 成列（`xl:grid-cols-[minmax(0,1fr)_360px]`），窄屏顺序排在正文之后、不做抽屉。
...
### data-testid 命名契约（e2e 依赖的 UI 契约，v1.13 首度成文）

`data-testid` 是本仓**唯一一处跨进程的 UI 契约**：Playwright e2e（`tests/e2e/`、`tests/e2e-ui/`、`tests/e2e-packaged/`、`tests/e2e-shots/`）与组件单测（`tests/ui/`）都按它点名元素。改一个名字，测试照样编译、应用照样跑——**只有那条用例会红**；而 e2e 点过的名字有一百多个，所以这套约定值得写下来，而不是散在大家脑子里。

**命名规则**：`<domain>-<element>[-<stable key>]`（全小写 kebab-case）。domain 是屏或组件的短名（`title`/`worlds`/`tree`/`settings`/`assets`/`engine`/`boot`/`protagonist`/`crafting`/`characters`/`creation`/`dialogue`/`bg`/`portrait` …），element 是部件或动作（`-screen`/`-notice`/`-error`/`-back`/`-continue`/`-import`/`-menu` …），`<stable key>` 是那一行/那一条的领域标识。真实代码里出现的是这几种形态（不是发明出来的理想）：

- **静态**：`title-continue`、`settings-muted`、`dialogue-box`、`boot-retry`。
- **带稳定 key**：`world-row-${worldId}`、`tree-node-${node.id}`、`character-card-${c.name}`、`preset-export-${current.id}`——key 取**稳定的领域标识**，同一份数据重取还是同一个 testid。
- **模板前缀**：`engine-backend-${e.id}`、`engine-${group}-mode-${m.value}`（group ∈ llm|image）、`` `${testIdPrefix}-zoom-in` ``（`CanvasZoomToolbar` 给 `tree`/`genealogy` 两块画布复用同一套后缀）。
- **刻意共用同一批**：两块画布（剧情图 / 家谱）的缩放控件、两块菜单（世界线行的 `⋯`、命令轨的分组菜单）走同一份命名，好让同一组用例钉住两边。

**稳定 vs 附随**：**e2e 直接点到的都是稳定契约**——`tests/e2e-ui/**` 里点过名的（一百来个）**改名 = 破坏性改动**，必须同批改用例并在 PR 里点明。**没人点名的**（只给组件单测或调试看的，如某个纯展示的内层节点）算附随，可以随手改。一条纪律不分稳定/附随：**同一个 testid 只该有一种语义**，新增时先全仓搜一遍别撞车。

**曾嵌不稳定 key 的三处（两处早先点名 + 一处同类未点名；v1.13 已收口）**：

- `tree-chapter-${key}`（章节切换器，`StoryTreeScreen.tsx`）：键来自 `lib/tree-view` 的 `chapterItems`——**章号在本章列表里唯一时就是章号本身**（`tree-chapter-2`）；只有章号撞号（只有标题、没有节点的章退化成 `index + 1`，撞上下一个真实章号）才退回 `` `${no}-${i}` ``（`tree-chapter-3-1` / `tree-chapter-3-2`），两颗药丸因此逐项唯一、都点得开。**诚实后果**：某个章号从唯一**变成**重号时，那一章的键会跟着变——`StoryTreeScreen` 的 `chapterIdx` 因此在精确键找不到时按章号认回第一颗同号药丸（再找不到才退回默认章），玩家的选中态不会白白漂走。
- `tree-list-group-${slug}`（降级列表的状态分组，同文件）：slug 来自 `lib/tree-view` 的 `STATUS_SLUG`（已走过→`visited` / 可达→`reachable` / 已剪枝→`pruned` / 嫁接→`grafted`）——testid 不再绑在**展示用词**上，哪天节点状态的中文用词变了也不会静默改键。
- `tree-archive-${archiveKey(line, i)}`（归档药丸，同文件）：键取该行 `第 N 章` 的章号（`第 1 章…` → `tree-archive-1`）；行里抽不出章号才退回 `line-${i}`（下标是这时唯一可用且不撞的兜底）。

**一条通则**：**枚举类键一律用 ASCII slug**（`STATUS_SLUG` 这类）；**内容派生的键在此是正当的——当它就是该事物的领域身份**（节点 id、角色名、资产名、章号、归档章号），同一份数据重取还是同一个键。另有一批**与屏无关的旧名**（`status`、`options`、`auto-advance`、`pending-prompt-hint`、`option-hints`、`shell-page`、`chapter-card`、`sr-status`、`skip-preload`、`quick-start*`）**登记为可接受形态**——它们不带 domain 前缀但已稳定，改名是**纯破坏性**改动，明确不做。

**撞脸但不真撞的几对**（同名不同域，写用例时按前缀区分）：

- 命令轨的 `settings`（game 屏右轨第一项，`TopBar.tsx`）vs 设置屏的 `settings-screen`（`SettingsScreen.tsx`）——前者是「打开设置」按钮，后者是设置屏根。
- 标题屏「更多」菜单里的 `preset-check`（`TitleScreen.tsx`）vs 体检屏的 `preset-check-screen` / `preset-check-*`（`PresetCheckScreen.tsx`）——前者是菜单项，后者是整屏及其内容。
- 同类还有命令轨的 `tree` / `characters` / `assets`（打开 overlay 的按钮）vs 各 overlay 内部的 `tree-*` / `characters-panel` / `assets-*`（overlay 自己的部件）。
...
测试：`tests/ui/` 给 jsdom 手工挂/删 `window.matchMedia` 垫片，覆盖「reduce 时整段显示且设置档位不动」与「matchMedia 缺失不降级」两条；`tests/e2e-ui/reduced-motion.spec.ts` 用 Playwright 的 `browser.newContext({ reducedMotion: "reduce" })` 走开局回合，断言长正文整段立现（无逐字过程）且动效降级不破坏 crafting→game 的屏切换。
...
- **本地端点只挡跨站浏览器请求（v1.6）**：来源校验针对的是浏览器发起的跨站请求（CSRF、端口探测），**不防本机其他进程**——本机任意脚本都能直接调 `/api/worlds`、`/prompt` 等端点，这里没有 token、没有登录态校验（`/api/auth` 只回当前引擎的登录文件是否存在、有没有自备凭据、能不能一键登录：`{loggedIn, hasCredentials, engine, canLogin}`）。它是「本机专用服务」这个前提下的最小防护，不是鉴权层。
...
| `src/store/game.ts` + `src/store/slices/*` | 事件编排（v1.6 按 slice 分文件，入口仍是 `store/game.ts`；跨片共享闭包与定时器在 `store/context.ts`——**v1.13 起两个定时器按 store 实例持有**，收尾口 `ctx.dispose()` / `game.ts` 的 `disposeStore()`）：段过滤重置逻辑、标记→画面应用、`expression` 表情切换与 `presetAdded` 刷新、章节制作流水线推进（规划→清单→队列→开演→【章】切章）、画廊单项/批量重绘与批量删除、创作装配状态机、`awaitCommand` 切屏；世界线（`beginNewWorld`/`resumeWorld`/`updateWorld`/`importWorldText`）与剧情图 overlay（`openTree`/`forkAt`/`treeEdited`/`restoreSnapshot`、编辑回合不进历史）；v1.6 设置（`updateSettings`）与自动前进（`armAutoAdvance`） |
| `server/acp-server.mjs`（入口与装配） | `startServer` 闭包**只做装配**、不再持任何逐回合状态（回合那摊 v1.13 拆去 `turn-pipeline.mjs`、资产那摊拆去 `assets-pipeline.mjs`、SSE 广播拆去 `sse.mjs`）：建 `createTurnPipeline()` 注入能力（`broadcast`/`persistAsset` 一族/两个惰性 getter `getSession`/`getEngine`），把 `turns.onChunk`/`turns.onSeg` 直通 `createAcpSession`、`restartAcp` 用 `turns.isBusy()` 开闸与 `turns.resetEffort()` 复位、路由链转手 `turns.sendPrompt` 与 `currentPresetId` getter；`buildAcp`（每次 spawn 现读凭据）、`migrateLegacyState`/`migrateWorldsSchema` 启动迁移与端口重试/SIGTERM 收尾仍在此。`EFFORT`/`EFFORT_PLANNING` 真源在 `config.mjs`、`pickEffort`/`isDirectivePrompt`/`parseWorldRef` 真源在 `turn-pipeline.mjs`，入口只 re-export；主题取值真源在 `shared/theme.mjs`，入口只 re-export；拆出模块的符号由入口逐名 re-export——外部 import 面不变（见 ADR-0013） |
| `server/turn-pipeline.mjs` | `createTurnPipeline({ broadcast, persistAsset, retryPendingWithPreset, retryPendingWithOwnPreset, getSession, getEngine })`（v1.13 从入口 `startServer` 闭包拆出）：**一个回合从提示词到落定的全部记账**——持有逐回合七份状态（seg/busy/turnText/artScanPos/currentPresetId/currentWorldId/lastEffort）与十个回合函数（handleArtLine/ingestChunkText/flushArtLines/sniffPreset/applyEffortValue/applyEffort/isMainTurn/writeTurnSnapshot/supplementMissingOptions/sendPrompt），返回 `{ sendPrompt, onChunk, onSeg, isBusy, resetEffort, currentPresetId }`。依赖直接 import（snapshots/worlds/protocol-lines/shared/assets/errors/config），只有会话与引擎是**惰性 getter 注入**——`restartAcp` 会换掉它们；`handleArtLine` 分流顺序即契约（art → expression → tree → presetAdded → audio）。`pickEffort`/`isMainTurn`（引用 shared 的 `DIRECTIVE_PREFIX_RE`）与质量守卫的章末豁免（`supplementMissingOptions` 引用 `CHAPTER_MARK_RE`）都钉在本文件源码（契约 lint ⑥ 抓函数体） |
...
| `server/presets.mjs` | preset.md 解析（`FM_KEYS`/`THEME_KEYS`/`parseFrontmatter`/`normalizeTheme`/`parseCharacterSections`/`parseSectionLines`）、`assetTargetFile`（封面按标题反查所以在这里）与剧本导出包（`buildPresetBundle`/`importPresetBundle`，见「剧本导出包」）；主题白名单/兜底值 v1.13 起 import 自 **`shared/theme.mjs`**（唯一真源；此前反向 import 入口、acp-server ↔ presets 成环） |
...
| `server/worlds.mjs` | 世界线索引与 CRUD：`readWorldsIndex`（裸数组与 `{schema, worlds}` 两种顶层形态都认）/`writeWorldsIndex`（写 `{schema:1, worlds}`，读改写保留未来 `schema` 与未知顶层键——ADR-0018）/`listWorlds`/`createWorld`/`forkWorld`/`restoreWorld`/`updateWorld`/`deleteWorld`（`moveToTrash` 唯一回收站入口，ADR-0014）/`exportWorld`（刻意不含 logs/）/`importWorld`/`migrateLegacyState`（旧扁平布局）/`migrateWorldsSchema`（索引 schema，启动期调用）；角色面板解析 v1.13 已拆去 `state-view.mjs` |
| `server/audio.mjs` | `scanPresetAudio`（`presets/<id>/audio/` 扫描；`AUDIO_FILE_RE`/`AUDIO_REL_RE`/`AUDIO_MIME` 真源在 `shared/protocol.mjs`） |
| `server/http-util.mjs` | 本地端点两道闸（`isCrossSiteRequest`/`readBodyText` + `MAX_BODY_BYTES`；可选 `maxBytes` 唯一消费方是剧本导入 50MB）、出网小工具 `withTimeoutSignal`/`joinEndpoint`（三处带超时的 fetch 收口）与 `/app` 静态托管的 `MIME`/`resolveAppDist` |
...
| `server/sse.mjs` | `createBroadcaster()`（v1.13 从入口闭包拆出）：`/events` 长连接集合 + `addClient`/`removeClient`/`broadcast(obj)`（只写 `data: <json>\n\n`，同步不 try/catch）/`clientCount`/`closeAll`。**收尾必须 destroy 每条连接**，否则 `server.close()` 回调永远不返回（打包态 e2e 实测抓到） |
| `server/assets-pipeline.mjs` | `createAssetPipeline({resolveImage})`（v1.13 从入口闭包拆出）：资产注册表 + `persistAsset`/`persistAssetFromFile` 落盘（`resolvePersistPreset` 是唯一「落哪个剧本」判定；封面走 `presets/<id>/cover.jpg`）+ `listAssets(presetId)` 画廊清单（每条回填 `preset`；`inUse` 只扫该剧本的世界）+ `retryPendingWithOwnPreset`/`retryPendingWithPreset`（回合末补扫 / 【新剧本】补落）+ 两处告警去重集。**不认识「回合」**——会话图片定位由 `resolveImage` 注入 |
| `server/settings-api.mjs` | `createSettingsApi()`（v1.13 从入口闭包拆出）：设置屏/启动屏的服务端面——`credentialsView`/`updateCredentials`/`testCredentials`（凭据增删查测）+ `startEngineLogin`/`logoutEngine`（登录登出）+ `providersView`/`allowedProviderIds`（服务目录读取与白名单）。**每次调用都现读凭据、不缓存**（「保存后立刻测试连接」要读到新值）；只碰磁盘配置、不碰会话（重启引擎是客户端连打两个端点） |
| `server/state-view.mjs` | 角色面板解析与视图（v1.13 从 `worlds.mjs` 拆出）：`parseStateFile(text)`（state.md 容错解析——小节缺/乱序/值越界都不抛，未知小节与未知键忽略）、`stateViewFor(worldId, root)`（`GET /api/state` 路由判定 400/404/200，root 可注入单测） |
| `server/fs-guard.mjs` | `withinRoot(abs, root)` 路径越界守卫（v1.13 收口）：两侧 `path.resolve` + 比较串带 `path.sep`（`/game-evil` 不算在 `/game` 内、根本身不算「在内」）；`/app`·`/img`·`/audio` 三条直服与 media-mcp 落盘共用这一份（此前四处各写一遍） |
| `server/errors.mjs` | `errText(e)`/`errName(e)`（v1.13）：`useUnknownInCatchVariables` 打开后 `catch (e)` 的 `e` 是 `unknown`，20 处「日志/错误串写什么」的口径收在这一处；零依赖叶子（不 import 其它 server 模块，不进任何环） |
| `server/engines.mjs` | 引擎后端**行为描述符**（v1.11，ADR-0022）：`ENGINE_DESCRIPTORS`（`spawn`/`rulesFor`/`sessionMeta`/`loginFile`/`prepare`（codex 的 CODEX_HOME 准备）/`sessionImagesRoot`/`effortOption`/`authCmd`/`authAvailable`）、`engineFor`、`prepareSpawn` 与**唯一的跨平台 spawn 收口** `windowsSafeSpawn`（`.cmd`/`.bat` → 整条命令行 + `shell:true`）；`server/acp.mjs` 不出现品牌字面量 |
| `server/engine-auth.mjs` | 登录/登出编排（v1.11 收尾，ADR-0022）：`startLogin`（单例在途登录 + 输出尾巴 ≤40 行 + 回执 `hint`）、`runLogout`（执行 CLI 自己的 logout、删游戏侧 codex 副本、20s 上限）、`killPendingLogins`（收尾）；**登录态始终是玩家的**——游戏只把 CLI 叫起来 |
...
| `shared/engines.mjs` | 引擎展示真源（v1.11，ADR-0022）：`ENGINES`/`ENGINE_IDS`（由 `ENGINES` 派生）/`DEFAULT_ENGINE_ID`/`engineById`（GUI 只渲染；契约 lint ⑧ 组钉 id 集合与 `server/engines.mjs` 描述符表一致、不用字面量）；手写声明 `shared/engines.d.mts` |
| `shared/theme.mjs` | 主题取值唯一真源（v1.13）：`MOTIFS`/`FONT_PRESETS`/`DIALOG_TEXTURES`/`FALLBACK_THEME`——`server/presets.mjs` 与 `src/theme.ts` 都 import 它（**环已消除**：此前值钉在入口、`presets.mjs` 反向 import）；手写声明 `shared/theme.d.mts`（字面元组派生三个联合类型）；`global.css` 的 `--accent`/`--accent2` 初始值是唯一另一处字面（CSS 没法 import，契约 lint ⑥ 组同批钉） |
| `server/routes.mjs` | `createRequestHandler(ctx)` HTTP 路由链（闭包能力由入口注入）：`/api/presets`(GET,POST:import)/`/api/presets/export`、`/api/auth`、`/api/providers`（v1.10）、`/api/credentials`(GET,POST)/`/api/credentials/test`/`/api/engine/restart`/`/api/engine/login`(POST)/`/api/engine/logout`(POST)、`/api/assets`(GET,POST:删除)、`/api/audio`/`/audio`、`/api/worlds`(GET,POST 全 action)/`/api/worlds/export`、`/api/history`、`/api/tree`、`/api/state`、`/img`、`/events`、`/prompt`、`/app` 静态托管（端点语义见「HTTP / SSE API 一览」） |
| `server/config.mjs` | 路径与端口常量：`GAME_ROOT`/`BASE_PORT`/`PORT_MAX_RETRY`/`SESSION_FILE`/`WORLDS_ROOT`、`gameHome()`（主目录，`BUNKITEN_HOME` 可覆盖）与推理档位常量 `EFFORT`/`EFFORT_PLANNING`（v1.13 从入口搬来：回合流水线要用又不能反向 import 入口，与路径/端口同处零依赖叶子；入口只 re-export） |
| `shared/protocol.mjs` | 协议常量唯一真源（v1.7，见 `docs/adr/0012`）：`PROTOCOL_HEADS`（9 头）、`AUDIO_KINDS`/`AUDIO_EXTS`/`AUDIO_MIME`/`AUDIO_FILE_RE`/`AUDIO_REL_RE`（音频白名单与直服正则，后两者由前两者构造）、`ART_KINDS`/`ASSET_KINDS`/`ASSET_FILE_RE`（美术类型字面与资产文件名正则，v1.7 收尾收编；`ASSET_FILE_RE` 由 `ASSET_KINDS` 构造）、`DIRECTIVE_PREFIX_RE`（指令前缀正则，`pickEffort` 推理分档与 `isMainTurn` 正戏回合判定共用）。`src/lib/parser.ts` re-export（公共 API 不变）并从真源构造 `ART_LINE_BODY`/制作清单正则、`server/turn-pipeline.mjs` import（`DIRECTIVE_PREFIX_RE`/`CHAPTER_MARK_RE` 的消费点在这；入口 `acp-server.mjs` 只 re-export `AUDIO_KINDS`/`AUDIO_EXTS`/`ASSET_FILE_RE`/`ASSET_KINDS` 给 `scripts/doctor.mjs`）；`shared/protocol.d.mts` 是手写类型声明（tsc -b 按 `.mjs`→`.d.mts` 解析；vite/vitest/electron 运行时直接吃 `.mjs`）。`RULES` 刻意不收编：引擎只读 `.grok/` 提示词、不会 import 代码，所以它是两份独立文本——server 常量 ↔ 本文档的副本逐字比对，server ↔ `SKILL.md` 按共有不变量锚点比对（契约 lint ② 组） |
| `tests/parser.test.ts`、`tests/crafting.test.ts`、`tests/server.test.ts`、`tests/treeLayout.test.ts`、`tests/genealogy.test.ts`、`tests/diff.test.ts`、`tests/doctor.test.ts`、`tests/preload.test.ts`、`tests/credentials.test.ts`、`tests/providers-catalog.test.ts`、`tests/ui/`、`tests/integration/*` | **单测 + 集成全量 645+ 例**（**下限口径**：数字说的是「不少于」，唯一维护点是 `tests/contract.test.ts` 的 `CASE_GROUPS`/`CASE_TOTAL`——加用例不用改任何文档、删用例才红，逐分组数字不再抄进文档）——契约字符串快照与单测：开局指令四变体（含世界段）/续玩/剧情编辑、分项美术/重绘/章节规划/创作模式指令、`开演。`、清单（含差分项）/章标记与选项解析期望值（parser）、章节制作流水线指令序列（crafting，stub fetch）、世界线/资产落盘判定/协议解析/快照与导出导入/剧本导出包（往返/重名/文件名安全/扩展名与 base64 校验）/state.md 容错解析与 `/api/state` 路由判定、回合日志 `writeTurnLog` 与追问指令常量、索引 schema 与迁移（裸数组升起、写路径写 `{schema:1, worlds}`、未来 schema 读到且写回不降级——ADR-0018）（server，含剧本体检端点）、布局纯函数（treeLayout + genealogy：家谱森林分层、孤儿 missingParent、fork 环终止、层内排序确定性与键盘步进）、快照对比纯函数（diff：全等/全增/全删/替换块相对顺序/空输入/空行/典型 state.md 好感度一行）、剧本体检查纯函数（doctor：tmp 根造 preset 覆盖七组判定、theme 双层回退与 checkAllPresets 汇总，见「剧本体检查」节）、引擎凭据纯函数（credentials：读写往返与 0600、坏 JSON 容错、掩码边界、`credentialsToEnv` 每模式、`publicView` 不含明文、服务目录表完整性、media-mcp 的尺寸/端点/目标路径/取字节/参数退让、探活的两条路径——ADR-0019）、服务目录在线通道纯函数（providers-catalog：整包拒 / 单条丢的两级取捨、https-only 与本机 http 例外、白名单字段重建、目录 0700 / 文件 0600 原子写、坏缓存当无缓存、缓存优先与进程内 memo、先主后备与 TTL 节流、两个开关——ADR-0020）、组件含设置屏、自动前进与回退后分割线/待重同步（含在途中止）、动效降级打字机、主题字体族与对话框质感、重演这一幕全链（含快照数不足时入口不渲染）、角色面板渲染/秘密折叠/turn_end 重拉/空态、世界线家谱视图、快照对比面板、标题屏剧本导出/导入、同屏多立绘的队列渲染与让位档（ui）；立绘差分预热纯函数（preload：清单每剧本只拉一次、命中角色的基础与全部差分都进预载、失败静默）；另有真 server 子进程的集成测试（`integration/pipeline`、`integration/audio-history`、`integration/http-guard`、`integration/credentials`——音频事件、快照落盘与精确回退、世界线与剧本的导出/导入往返、索引 schema 的启动迁移（裸数组升起 / 未来 schema 读到且不被降级写回）、来源校验 403 与 body 上限 413、引擎 error response 的 409 传播、回合原文日志与缺选项段的质量守卫追问；自备 key 则覆盖：端点脱敏与 400 校验、盘上 0600、env 真注进引擎子进程（假引擎探针）、图片侧才挂 MCP 与重启后重 spawn、`/api/auth` 的三态、`/test` 的两条探活路径、media-mcp 子进程的 initialize→tools/list→tools/call 冒烟；服务目录通道则用本地 mock 发布源验三态——启动抓远端（`source:"remote"`）→ 落盘 0600/目录 0700 → 断源重启吃缓存 → 全新 HOME 抓不到回内置兜底且不写缓存（`integration/providers-catalog`，harness 为此加了 `extraEnv`/`homeDir` 两个可选参数并默认钉 `BUNKITEN_DISABLE_UPDATE=1`）；mock 出图链路（`integration/media-mock`：假引擎真发 `tools/call` → media-mcp → `tests/helpers/mock-image-server.mjs` 的本地假图片服务 → 同名 jpg 字节落盘 + 【图】事件）） |
| `tests/contract.test.ts` | v1.6 起契约 lint（防漂移门禁，读源码与文档、不起子进程）：`PROTOCOL_HEADS` 唯一真源（`shared/protocol.mjs` 源码字面 + import 值 + parser re-export 链三方钉住）↔ server/parser 解析出口 ↔ `SKILL.md`「标记格式备忘」/本文档、`RULES` 逐字副本（server 常量 ↔ 本节代码块）、指令字符串双处存在（`parser.ts` ↔ `SKILL.md`）、`DIRECTIVE_PREFIX_RE` 单一真源（`pickEffort`/`isMainTurn` 函数体都引用它、server 无第二份前缀字面）、主题白名单与兜底主题（server ↔ `src/theme.ts` 同集同值）、各测试文件的 `it(`/`test(` 用例数不低于 `CASE_GROUPS` 声明的**分组下限**（下限语义：加用例不用改任何文档，删用例才红；文档只留一句粗口径、不再逐分组抄数字；v1.13 起还断言**每个测试文件都在某个分组里**——整份文件被删也要红）、`RULES` ↔ `SKILL.md` 的共有不变量锚点（两份独立文本必须同意同一批事实）、`shared/*.d.mts` 手写声明与运行时导出名双向对齐（`tests/declarations.test.ts`）、设置键 `bunkiten.settings.v1` 与音频扩展名三处一致（音频常量断言指向 `shared/protocol.mjs` 真源）、美术类型集合与资产文件名正则真源（`ART_KINDS`/`ASSET_KINDS`/`ASSET_FILE_RE`：双侧构造 + server/scripts 无第二份字面）、引擎凭据（v1.10，⑦ 组）：服务目录表完整性与「GUI 吃 `shared/providers.mjs` 真源、src/ 不许出现目录 id 字面量」、两组模式字面与 server 的 `LLM_MODES`/`IMAGE_MODES` 同词、MCP 工具名（`media-mcp.mjs` 的两个常量拼出的 `bunkiten-media__generate_image` ↔ SKILL.md 的「出图工具优先」句）、凭据落点与环境变量名 ↔ 本文档逐字一致、服务目录发布源 `docs/providers.json` ↔ `PROVIDERS` 深等（v1.10：手改 JSON 会红，跑 `npm run providers:export` 重新生成）。**它自己的用例不计入合计下限口径**（`tests/e2e/**` 同样不在口径内）。界面截图另有 `tests/e2e-shots/`（`npm run shots`，作者侧、不进 CI，产出 `docs/images/`） |

v1.3 三组新契约的同步点速查（同一改动五处联动的具体落点）：

| 契约 | 同步点 |
|---|---|
| 差分与表情切换（`【清单】立绘\|<角色>-<变体>` / `美术：立绘 <角色>-<变体>` / `【立绘】<角色>\|<变体>`） | `SKILL.md`【章节与剧情树】差分清单、【每轮协议】【美术】差分规则与导演层第 8 条 · `parser.ts` `splitAssetVariant` · `protocol-lines.mjs` `parseExpressionLine`（`turn-pipeline.mjs` `handleArtLine` 广播 `expression`）· `store/game.ts` `nextPortraitOnExpression` · `PortraitLayer` 两级回退 · `tests` 快照 |
| 重绘（`美术：重绘 <类型> <名>[-<变体>]` 与【图】第四段「重绘」） | `SKILL.md`【素材重绘】+ `RULES` 第 3 句 · `parser.ts` `buildRegenCommand`/`scanMarkers` 四段正则 · `protocol-lines.mjs` `parseArtLine` regen 分支与 `assets-pipeline.mjs` `persistAsset` 覆盖 · `store/game.ts` `startRegen`/`finishRegen` · `tests` 快照 |
| 创作模式（`创作模式：进入剧本创作。` / `装配。` / `【新剧本】<id>`） | `SKILL.md`【剧本创作】+ `RULES` 第 3 句 · `parser.ts` `ENTER_CREATION`/`BUILD_ASSEMBLE`/`isProtocolLine` · `protocol-lines.mjs` `parsePresetAddedLine` → `turn-pipeline.mjs` 广播 `presetAdded` 事件 · `store/game.ts` creation 状态机 · `tests` 快照 |
...
| 剧情编辑（`剧情：<指令>` / `【树】` 行） | `SKILL.md`【剧情编辑指令】· `parser.ts` `buildTreeEditCommand`/`parseStoryTree`/`isProtocolLine` · `protocol-lines.mjs` `parseTreeLine` → `turn-pipeline.mjs` `handleArtLine` 广播 `treeEdited` 事件 · `store/game.ts` tree overlay 与「编辑回合不进历史」 · `tests` 快照 |
...
| 契约 | 同步点 |
|---|---|
| 音频三行（`【曲】<名>` / `【环境】<名>` / `【音效】<名>`） | `SKILL.md`【音频】+【导演层】第 9 条 + `RULES` 第 6 句（音频纪律） · `shared/protocol.mjs` 的 `PROTOCOL_HEADS`（9 项）/`AUDIO_KINDS`（唯一真源；`parser.ts` re-export 并构造 `isProtocolLine`）· `protocol-lines.mjs` `parseAudioLine` → `turn-pipeline.mjs` `handleArtLine` 末位分支 → `audio` 事件、`audio.mjs` `scanPresetAudio`/`AUDIO_FILE_RE` 与 `routes.mjs` 的 `/api/audio`、`AUDIO_REL_RE`/`AUDIO_MIME` 与 `/audio`（常量 import 自 shared）· `lib/audio.ts` `AudioManager`（「类型\|名 → 文件」索引、缺失静默）· `store/slices/gameplay.ts` `handleEvent` 的 `audio` 分支 · `lib/settings.ts` 音量键 · `tests/integration/audio-history.test.ts` 集成断言 |
| 快照与回退（`state/worlds/<worldId>/history/NNNN.json` 路径与条目结构） | `snapshots.mjs` `writeSnapshot`/`readSnapshot`/`readSnapshots`/`normalizeSnapshot`/`latestSnapshot`/`selectSnapshotForNode`/`writeWorldFiles`（null=删文件）与 `worlds.mjs` `forkWorld(seq)`/`restoreWorld`/`exportWorld`/`importWorld`、`routes.mjs` 的 `/api/history`、`/api/worlds {fork,restore,import}`、`/api/worlds/export` 端点 · `lib/acp.ts` `fetchHistory`/`fetchSnapshot`/`postWorldRestore`/`worldExportUrl`/`postWorldImport` · `store/slices/tree.ts` `restoreSnapshot`（成功后自增 `treeStamp` 并补发 `继续世界：<worldId>。`）/`forkAt` · `StoryTreeScreen` 节点「存档点 · 第 N 幕」标注（玩家侧文案；裸 `快照 #N` 与「轮/回合」不上屏）、两段确认回退与「回到这一幕并重演」（出现条件 `lib/replay.ts` 的 `canReplaySnapshot`）、有快照的分叉带 seq · `tests/server.test.ts` + `tests/integration/audio-history.test.ts` |
| 世界线 `label` / `note` | `state/worlds/index.json` 字段 · `worlds.mjs` `updateWorld`（≤60 / ≤200，空串=清除）/`importWorld`（note 追加「（导入）」）/`forkWorld`（血缘只写 `forkedFrom` + `fork.md`，`note` 恒空串——旧版自动写的「分叉自 …」由 `lib/worlds.ts` 的 `isLegacyForkNote` 判为「没有备注」）/`labelSnapshot`（v1.12：存档点名字存索引 `snapshotLabels`，≤40 字、空串清除）/`listWorlds`（老索引补 `label:""`） · `lib/acp.ts` `postWorldUpdate`/`postSnapshotLabel` · `store/slices/world.ts` `updateWorld`/`importWorldText`/`parseWorldBundle` · `WorldsScreen` 行上 `⋯` 菜单里的改名编辑器与 `worldDisplayName`（`label → note（旧分叉备注不算）→ 所属剧本标题 →「未命名世界线」`）· `tests/server.test.ts`+`tests/ui/` |
| 设置键（`bunkiten.settings.v1`） | `lib/settings.ts` `SETTINGS_STORAGE_KEY`/`DEFAULT_SETTINGS`/`normalizeSettings`/`loadSettings`/`saveSettings`/`TEXT_SPEED_MS`/`AUTO_ADVANCE_OPTIONS` · `store/slices/nav.ts` `updateSettings`（唯一写入方：AudioManager + localStorage）· `SettingsScreen` 控件 · `DialogueBox`（文字速度）/`OptionList`（自动前进）· `App.tsx`（启动时把设置喂给 AudioManager）· `tests/ui/` |
...
改完跑 `npm test`（单测 + 集成；改契约字符串必须同步快照与 `tests/contract.test.ts` 的三处断言）、`npm run build` 与 `npm run typecheck:server` 验证类型与构建。

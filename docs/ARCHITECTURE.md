...
| `components/EngineKeysSection.tsx` | 「引擎与密钥」设置节（v1.10，ADR-0019）：服务目录下拉（吃 `shared/providers.mjs` 真源）+ 地址/密钥/模型，出图多一格「出图尺寸（高级）」；「测试连接」三态（测试中 / 通过并给延迟 / 失败给脱敏原因）；保存即时生效；「立刻重启引擎」= `POST /api/engine/restart`。文案是玩家话（服务地址/密钥/模型），不出现 env 与变量名 |
...
| 世界线 `label` / `note` | `state/worlds/index.json` 字段 · `worlds.mjs` `updateWorld`（≤60 / ≤200，空串=清除）/`importWorld`（note 追加「（导入）」）/`forkWorld`（血缘只写 `forkedFrom` + `fork.md`，`note` 恒空串——旧版自动写的「分叉自 …」由 `lib/worlds.ts` 的 `isLegacyForkNote` 判为「没有备注」）/`labelSnapshot`（v1.12：存档点名字存索引 `snapshotLabels`，≤40 字、空串清除）/`listWorlds`（老索引补 `label:""`） · `lib/acp.ts` `postWorldUpdate`/`postSnapshotLabel` · `store/slices/world.ts` `updateWorld`/`importWorldText`/`parseWorldBundle` · `WorldsScreen` 行上 `⋯` 菜单里的改名编辑器与 `worldDisplayName`（`label → note（旧分叉备注不算）→ 所属剧本标题 →「未命名世界线」`）· `tests/server.test.ts`+`tests/ui.test.tsx` |

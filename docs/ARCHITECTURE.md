...
**不进 CI（取舍）**：CI 的 `npm test` 已守 doctor 纯函数判定，而「仓库此刻 preset 的健康度」随游玩数据（state/worlds、素材增删）漂移——孤儿素材这类 warning 依赖本机世界进度，门禁化会误伤；它是作者侧工具，改 preset 结构或新增剧本后按需跑。

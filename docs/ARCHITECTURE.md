...
环境变量：`GROK_GAME_ROOT`（游戏数据根）、`PORT`（起始端口）、`EFFORT`（推理档，默认 `medium`）、`ACP_PROXY_TARGET`（仅 dev 代理目标）、`BUNKITEN_HOME`（覆盖主目录——测试与打包态冒烟把 home 指到临时目录；**Windows 上不能改 `USERPROFILE`**，Chromium 读它）。
...
| `lib/tree-view.ts` | 剧情图屏展示层纯函数（v1.13 从 StoryTreeScreen 收编，不 import React/DOM）：`chapterNoOf`/`chapterLabel`（章号取节点 id 的 `N-M` 前缀）、`snapshotTurnNo`、`earliestSnapshotByNode`、对比基线 `prevSnapshotSeq`；⚠️ 与 `lib/replay.ts` 的 `prevTurnSnapshotSeq` **同名不同义**（对比基线 kind 不限 vs 重演回退点只认 turn），别合并 |
...
**两处嵌了不稳定 key 的（此刻的债，点名在这里）**：

- `tree-chapter-${it.key}`（章节切换器，`StoryTreeScreen.tsx`）：`it.key` 是 `chapters.map((ch, i) => …)` 拼出的**复合键** `` `${no}-${i}` ``——**后半段是数组下标**。同一棵树重取时下标稳定，但**编辑树改动了章的数量或次序、或解析出的章集合变了**，下标段就会错位，用例按 `tree-chapter-1-0` 点的那一颗可能已经不是同一章。为什么不用章号单独当键：只有标题、没有节点的章会退化成 `index + 1`，与下一个真实章号撞号（见该文件注释）——这是个已知的两难，写在这里免得有人把它当成稳定标识。
- `tree-list-group-${g.status}`（降级列表的状态分组，同文件）：`g.status` 是**中文字面量**（`已走过`/`可达`/`已剪枝`/`嫁接`，`STATUS_ORDER`）。CJK 进 testid 本身能用，但它把 testid 绑在了**展示用词**上——哪天节点状态的用词变了，这个 testid 会静默变成另一个，用例得跟着动。这两处都该在日后换成稳定的 ASCII 键。
...
| `server/acp-server.mjs`（入口与装配） | `startServer` 闭包（资产那摊 v1.13 拆去 `assets-pipeline.mjs`、SSE 广播拆去 `sse.mjs`）：`handleArtLine` 分流（分支顺序 art → expression → tree → presetAdded → audio）、落盘纪律由 `createAssetPipeline()` 提供（`persistAsset`/`persistAssetFromFile`/`listAssets`——`resolvePersistPreset` 是唯一「落哪个剧本」判定；封面走 `presets/<id>/cover.jpg`；`listAssets` 每条回填 `preset`、`inUse` 只扫该剧本的世界）、`sniffPreset` 世界/剧本落定、`sendPrompt` 的正戏回合判定（`isMainTurn`）与快照/回合日志落盘点、质量守卫 `supplementMissingOptions`；`pickEffort`/`isMainTurn`（引用 shared 的 `DIRECTIVE_PREFIX_RE`）钉在入口源码（契约 lint ⑥ 抓函数体）；主题取值 v1.13 起真源在 `shared/theme.mjs`，入口只 re-export；拆出模块的符号由入口逐名 re-export——外部 import 面不变（见 ADR-0013） |
...
| `server/config.mjs` | 路径与端口常量：`GAME_ROOT`/`BASE_PORT`/`PORT_MAX_RETRY`/`SESSION_FILE`/`WORLDS_ROOT` 与 `gameHome()`（主目录，`BUNKITEN_HOME` 可覆盖） |

...
- **打包态 + 真 CLI + 真图片服务的「真画一张图」——已实现（v1.10）**：opt-in 真链路 `tests/e2e-packaged/real-image.spec.ts`（真 grok CLI + 本机登录态 + 真图片服务凭据 + 出网；走画廊重绘——SKILL 里唯一绕过生成前缓存的路径，真花一次对话与一张图；缺凭据/登录/产物即干净跳过，不进 CI），外加**三层 mock 覆盖**：集成层 `tests/integration/media-mock.test.ts` 与 e2e-ui 层 `tests/e2e-ui/media-mock.spec.ts`（**进 CI**）、打包态 `tests/e2e-packaged/packaged.spec.ts` 的第三条（**随打包冒烟一起跑**——v1.13 更正：它不是 opt-in，只在产物缺失且未设 `BUNKITEN_REQUIRE_PACKAGED=1` 时跳过；假引擎真调 MCP + 本地假图片服务，不需要真凭据/真网络）。三层替身各测协议面/UI 面/打包面，真链路负责证明「真能画出来」。
...
- **已知 flake（负载敏感）—— 两段账分开说**：
  ...
  - **点击那条（本轮收口形状，机制只证到一半）**：同一族窗口在**点击路径**上还有一次观测——三套测试 + 构建同时跑时出现「点缩放后 viewBox 被拉回适应」（子代理观测约 4 次 1 次）；上一轮 11 次复现尝试全绿，本轮按下面口径压机器一轮（构建循环 ∥ vitest 循环 ∥ 整条 e2e-ui 一起跑）也未复现，**负载下的栈至今没抓到**。本轮把两条可疑形状改掉：① `lib/useCanvasPanZoom` 的布局复位从被动 effect 的排队写入改成**渲染阶段调整**（复位与触发它的提交同拍落定；挂载也不再白写一次）；② `StoryTreeScreen` 的树重取（`treeStamp` 变）不再清 data、不再拆画布——视图与 DOM 身份都留着；换世界线仍由响应自带的 worldId 闸门立刻清屏，重取失败也保留旧树（只多一条错误提示）。**证据边界（别当结论用）**：确定性用例「refit 不留排队写入：画布落地那一拍就点缩放」（把被动 refit 放回去它就红）钉住的是 **①** 这条机制，「重取不拆画布」「重取失败保留旧树」两条钉住 **②**；但「负载下那次红就是 ①/②」**没有被证明**。
